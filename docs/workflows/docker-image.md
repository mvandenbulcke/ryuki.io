...
Operator runbook for the **Docker image build contexts** coverage entry. The platform serves a static, provider-safe contract for this slice; this page maps the contract to its catalog source, lifecycle, required inputs, prohibitions, and evidence expectations.

## Contract

- Contract definition `docker-image-contract.yaml`
- Serves contract route `/api/platform/release-promotion-contract`.
- Validator slice `docker-image`
- Contract `docker-image-contract.yaml` is marked draft (version 1)
