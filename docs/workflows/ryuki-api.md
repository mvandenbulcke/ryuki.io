...
Operator runbook for the **ryuki-api** coverage entry. The platform serves a static, provider-safe contract for this slice; this page maps the contract to its catalog source, lifecycle, required inputs, prohibitions, and evidence expectations.

## Contract

- Contract definition `ryuki-api-catalog.yaml`
- Serves contract route `/api/platform/summary`.
- Validator slice `ryuki-api`
- Contract `ryuki-api-catalog.yaml` is marked draft (version 1)
