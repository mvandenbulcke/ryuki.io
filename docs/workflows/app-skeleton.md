...
Operator runbook for the **App skeleton** coverage entry. The platform serves a static, provider-safe contract for this slice; this page maps the contract to its catalog source, lifecycle, required inputs, prohibitions, and evidence expectations.

## Contract

- Contract definition `app-skeleton-contract.yaml`
- Serves contract route `/health`.
- Validator slice `app-skeleton`
- Contract `app-skeleton-contract.yaml` is marked active
