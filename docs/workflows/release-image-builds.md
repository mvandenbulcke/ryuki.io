...
Operator runbook for the **Release image builds** coverage entry. The platform serves a static, provider-safe contract for this slice; this page maps the contract to its catalog source, lifecycle, required inputs, prohibitions, and evidence expectations.

## Contract

- Contract definition `release-image-builds-contract.yaml`
- Serves contract route `/api/platform/release-promotion-contract`.
- Validator slice `release-image-builds`
- Contract `release-image-builds-contract.yaml` is marked active
