...
| 16 | [x] | Enforced site degradation mode (write gating) | Resil | L | H | Migration 198 replaces the racy handler precheck with one canonical database epoch: live mint, persistence, lease, acknowledgement, renewal, and first result acceptance require an active, fresh `healthy/up/up` site plus fresh VMware observation. Unsafe or stale work is not dispatched; authority loss during execution retains signed evidence and becomes `ReconcileRequired`. The five-minute observation TTL is registered in the security-limit profile. Distributed control-plane epoch signing/failover remains tracked under SB-3/SB-8, not this local site-health item. |
...
degradation ENFORCEMENT (tracker #16) originally used
`enforce_site_operational` before grant mint (`5ae1430`). Migration 198
supersedes that precheck with a transaction-bound, monotonic site-health epoch
revalidated through the complete live-job lifecycle; configured-database status
reads also fail closed instead of consulting process-local fallback state.**
