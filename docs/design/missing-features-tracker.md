...
| 42 | [~] | Multi-step orchestration / job dependencies | Exec | L | M | Slice 1 `5459724` (pure ryuki-engine::job_orchestration: validate_plan/ready_steps/cycle-detect). Slice 2a `4473f68` — job_steps table (mig 151) + requests_execute materializes a request plan's INITIAL ready steps as agent_jobs (OfflineDryRun-only, non-offline mode rejected 400; single-job path byte-for-byte unchanged; all in the locked→executing CAS tx). GPT-5.5-Codex-xhigh: no defects. Remaining slice 2b: dispatch next-ready steps on step-success (post_job_result backlink) + public plan-authoring endpoint |
