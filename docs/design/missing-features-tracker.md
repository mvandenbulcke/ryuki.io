...
| 14 | [~] | List filtering/search + pagination envelope (API) | API | M | H | Slice 1: requests_list — filters (status/site/env/type/created_by/q) + allowlisted sort + limit/offset + X-Total-Count (`1f5ecfa`). Slice 2 `1bd4686` — bounded the unbounded networking-inventory lists (dns_records/ipam_subnets/firewall_rules): limit/offset + filtered total, non-breaking (existing keys kept, generous 500 default cap), scope-safe COUNT; GPT-5.5-Codex-xhigh clean, live-DB tested. Remaining: other unbounded list endpoints (admin sessions/tokens, secrets, patch waves, failure patterns, …) — same pattern, piecemeal. NOT doing the breaking {items,total} envelope (bare-array + X-Total-Count is the chosen shape) |
