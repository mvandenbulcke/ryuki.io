...
- **ReconcileRequired resolution endpoint** (H/M, product-ambiguous): `expire_leases` sets LiveApply
  ...
  Slice: `POST /api/admin/agents/jobs/{id}/reconcile` CAS `ReconcileRequired`→`Failed`, audited. ✅ SHIPPED — admin CAS→Failed + audited reason + a non-alerting `job.reconcile_resolved` event; parent request left Executing (operator /fails separately). Plan and implementation review APPROVED. See reconcile-required-resolve.md.
- **ReconcileRequired emits no alert + strands the parent request** (H/S): unlike the dead-letter
  ...
  Slice: emit a `job.reconcile_required` event in that branch, mirroring the dead-letter block. ✅ SHIPPED — emits the event + made `reconcile-required` alert-worthy (Critical) so the alert feed surfaces it (parity with dead-letter). Review APPROVED.
...
- **Integration-connection MUTATION handlers write no audit_log row** (H/S): only `integration_test`
  ...
  fixes the no-DB TOCTOU; secret-safe detail). Plan and implementation review APPROVED. See
  ...
  test); 6 DB tests incl. no-secret-leak + read-path. Plan and implementation reviewed. See
  ...
  state, since a healthy `closed` row can be persisted). ✅ THEME COMPLETE: every integration mutation (create / update /
...
- **TLS certificate expiry scan** (M/S) — `certificate_lifecycle`. ✅ SHIPPED (592942c) —
  ...
  (Expired→P1, ExpiringSoon→P2) + open-item REFRESH so soon→expired upgrades in place. Plan and
  implementation review APPROVED. See certificate-expiry-scan.md.
- **OOB-management cert-endpoint expiry scan** (M/S) — `oob_endpoints`. ✅ SHIPPED — `oob_cert_expiry_scan`
  ...
  Expired→P2/ExpiringSoon→P3 (internal mgmt access, not a public outage) + refresh. Plan and
  implementation review APPROVED. See oob-cert-expiry-scan.md. ✅ SCHEDULED-AUTOMATION THEME COMPLETE: all 6 expiry/overdue
...
- **gMSA service-account expiry scan** (M/S) — `gmsa_lifecycle::get_expiring`. ✅ SHIPPED (7028d4b) —
  ...
  + refresh; framed as "verify AD-side rotation". Plan and implementation review APPROVED. See gmsa-expiry-scan.md.
...
- **scheduler `job_executions` history prune** (M/S) — ✅ SHIPPED (720a1d0): `job_executions_prune`
  ...
  cap so a years-old backlog drains over days. Plan and implementation review APPROVED. See job-executions-prune.md.
- **`connection_health_checks` history prune** (M/S) — ✅ SHIPPED: `connection_health_checks_prune`
  ...
  (connection_id, checked_at DESC NULLS LAST, id DESC). Plan and implementation review APPROVED. See
...
- **Pending agent-job cancel** (M/M) — no way for an admin to cancel a queued job. ✅ SHIPPED —
  ...
  job-scoped (parent request left actionable; operator fails it via the existing `/fail`). Plan review
  found 2 blockers, both fixed; implementation review APPROVED. See pending-job-cancel.md.
