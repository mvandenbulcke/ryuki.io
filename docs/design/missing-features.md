...
> ## ⚠️ Reconciliation status — 2026-06-14, HEAD 763d4bb
>
...
- **Portal changes (`portal/portal-ui`):**
  - `shell.rs::is_authenticated()` stops returning `true`; it becomes a server-checked signal: a Leptos server function `get_auth_session()` reads the `ryuki_session` HttpOnly+Secure+SameSite=Lax cookie, validates it against `GET /api/auth/session`, and returns the real session or `None`. `app.rs`'s existing `if authenticated { Shell } else { LoginView }` gate then works as designed.
...
- `POST /api/audit/log/verify` — recomputes the hash chain over a range and reports the first divergent row, if any.
...
- **Releases**: all five crates are `0.1.0`; zero git tags local or remote; no CHANGELOG; images pinned as `ryuki/platform-api:rust-dev` / `ryuki/portal-ui:rust-dev` with `IfNotPresent` (`deployments.yaml:20-21,71-72`); `x-api-version: 0.1.0` hardcoded at `sources/ryuki-api/src/main.rs:546-548`; all 616 API routes unversioned (no `/v1`). `catalog/platform-release-promotion-contract.yaml` defines the full promotion process on paper (`dev-render` → `rollback-readiness`, validation signals `helm-lint`/`kustomize-build-render`) but is `status: draft` with `registryPushAllowed`/`helmUpgradeAllowed`/`kubectlApplyAllowed` all `false` and nothing to lint or render.
...
- **Changes**:
  ...
  - Replace `:rust-dev` image refs in `deployments.yaml` with digest pins applied via the F9 overlay; change `imagePullPolicy` to `Always` for tag-based dev overlays.
...
- **`orchestrate_reboot()` is unreachable**: implemented and tested (`patch_engine.rs:157`, tests at :702-756) but every other patch function is wired (`contracts.rs:4968-5010`) except this one.
...
- **Reboot orchestration** (single-file change): `POST /api/maintain/patch/reboot-orchestrate` near `patch_pending_reboots` (`contracts.rs:5009`), loading the wave from `patch_waves` (`migrations/010_patch_waves.sql`) and returning `patch_engine::orchestrate_reboot(&wave)` stages; add `reboot-orchestrate` to `patch_contract`'s `supportedWorkflows` (`:5023`).
...
5. **S** — Wire `orchestrate_reboot` (`POST /api/maintain/patch/reboot-orchestrate`) + contract workflow list.
