...
Status: design (round-one review changes folded in below). Reuses the
...
1. **Migration 119**: `ALTER TABLE requests ADD COLUMN next_maintain_review_at
   ...
   backfill, and NO change to requests_publish (avoids blast radius) — NULL
   semantics were clarified during review.
...
3. **API** `scheduler.rs` `run_job` new arm `"maintain_review_scan"`, ALL on the
   tick tx — ATOMIC claim+advance to race a concurrent retire safely (review fix):
   ```sql
   ...
   NO `to_status`, so it stays a NORMAL /api/events entry, not an alert — review
   ...
   make this structural (review note).

4. **No DbRequestRow / REQUEST_COLUMNS change**: the scan uses its own
