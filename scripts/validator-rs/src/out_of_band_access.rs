...
const CATALOG_PATH: &str = "catalog/out-of-band-access-validation-contract.yaml";
const API_README_PATH: &str = "api/Ryuki.Platform.Api/README.md";
const CATALOG_README_PATH: &str = "catalog/README.md";
const DOC_README_PATH: &str = "docs/workflows/README.md";
const DOC_PATH: &str = "docs/workflows/out-of-band-access-validation.md";

const ENDPOINT: &str = "/api/operations/out-of-band-access-validation-contract";

const REQUIRED_CONSOLE_TYPES: &[&str] = &["hpe-ilo", "dell-idrac", "lenovo-xcc"];
const REQUIRED_WORKFLOWS: &[&str] = &[
    "oob-access-readiness",
    "hardware-console-review",
    "oob-certificate-review",
...
const REQUIRED_BLOCKED_REASONS: &[&str] = &[
    ...
    "account-identifiers-disabled",
    "site-unknown",
    "hardware-profile-unknown",
    "access-profile-missing",
    "certificate-profile-missing",
    "role-model-missing",
    "break-glass-profile-missing",
...
const REQUIRED_RULES: &[RuleDetail] = &[
    RuleDetail {
        id: "no-live-oob-access-checks",
        decision: "block",
        requirement: "Out-of-band access validation reports review state only and never logs in to consoles, tests credentials, changes roles, or calls hardware controllers.",
        evidence: "OOB readiness summary",
    },
    RuleDetail {
        id: "certificate-readiness-required",
        decision: "block",
        requirement: "Certificate profile and renewal readiness must be reviewed before incident readiness can be accepted.",
        evidence: "Certificate readiness review",
    },
    RuleDetail {
        id: "role-model-review-required",
        decision: "block",
        requirement: "Role assignment intent and access profile must be reviewed without exposing account names or group identifiers.",
        evidence: "Role model review",
    },
    RuleDetail {
        id: "break-glass-readiness-required",
        decision: "block",
        requirement: "Break-glass procedure, ownership, and incident runbook linkage must be reviewed before readiness is accepted.",
        evidence: "Break-glass readiness review",
    },
    RuleDetail {
        id: "raw-oob-inventory-not-exposed",
        decision: "block",
        requirement: "OOB readiness evidence must use safe summaries only and must not expose endpoint names, hostnames, private IPs, serials, asset tags, account names, access group identifiers, raw inventory rows, or provider payloads.",
...
const REQUIRED_CATALOG_KEYS: &[&str] = &[
    ...
    "liveAccessChecksAllowed",
    "liveCertificateChecksAllowed",
    "rawInventoryRowsAllowed",
    "endpointIdentifiersAllowed",
    "serialNumbersAllowed",
    "accountIdentifiersAllowed",
...
const VARIABLE_ARRAYS: &[(&str, &str, &[&str])] = &[
    (
        "supportedConsoleTypes",
        "outOfBandAccessConsoleTypes",
        REQUIRED_CONSOLE_TYPES,
    ),
    (
        "supportedWorkflows",
        "outOfBandAccessWorkflows",
        REQUIRED_WORKFLOWS,
    ),
    (
        "readinessDomains",
        "outOfBandAccessReadinessDomains",
        REQUIRED_DOMAINS,
    ),
    (
        "requiredGuards",
        "outOfBandAccessRequiredGuards",
        REQUIRED_GUARDS,
    ),
    (
        "planSections",
        "outOfBandAccessPlanSections",
        REQUIRED_PLAN_SECTIONS,
    ),
    (
        "blockedReasons",
        "outOfBandAccessBlockedReasons",
...
const SAFE_RAW_CATALOG_COMMENTS: &[&str] = &[
    "Out-of-band access validation seed data only. Do not add endpoint names, hostnames, usernames, account names, access group identifiers, credentials, tokens, tenant IDs, object IDs, UUIDs, private IPs, raw inventory rows, serial numbers, asset tags, raw logs, or provider payloads.",
];
const SAFE_TEXT_PROHIBITION_LINES: &[&str] = &[
    ...
    "OOB readiness evidence must use safe summaries only and must not expose endpoint names, hostnames, private IPs, serials, asset tags, account names, access group identifiers, raw inventory rows, or provider payloads.",
];
const PROHIBITED_OOB_KEYS: &[&str] = &[
    "endpoint",
    "endpoints",
    "endpointname",
    "endpointnames",
...
const PROHIBITED_OOB_SUBSTRINGS: &[&str] = &[
    "endpoint",
    "hostname",
    "username",
    "accountname",
...
const SAFE_OOB_GUARD_KEYS: &[&str] = &[
    ...
    "serialnumbersallowed",
    "accountidentifiersallowed",
    "endpointidentifiersdisabled",
    "serialnumbersdisabled",
    "accountidentifiersdisabled",
...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    ...
    let context: OutOfBandAccessContext = serde_json::from_str(&payload)
        .map_err(|error| format!("invalid out-of-band access validation context JSON: {error}"))?;
...
pub fn validate_catalog_json(input: &str) -> Result<Vec<String>, String> {
    let catalog: Value = serde_json::from_str(input)
        .map_err(|error| format!("invalid out-of-band access validation catalog JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_program_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProgramInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid out-of-band access validation program JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_docs_json(input: &str) -> Result<Vec<String>, String> {
    let payload: DocsInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid out-of-band access validation docs JSON: {error}"))?;
    let mut errors = Vec::new();
    validate_docs_text(
        &payload.api_readme,
...
pub fn scan_prohibited_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProhibitedInput = serde_json::from_str(input).map_err(|error| {
        format!("invalid out-of-band access validation prohibited JSON: {error}")
    })?;
    let mut errors = Vec::new();
...
fn validate_catalog_value(catalog: &Value, errors: &mut Vec<String>) {
    let Some(map) = catalog.as_object() else {
        errors.push("out-of-band access validation catalog must be a mapping".to_string());
        return;
    };
    ...
    if !unexpected.is_empty() {
        errors.push(format!(
            "out-of-band access validation unexpected catalog keys: {}",
            unexpected.join(", ")
        ));
    }
    ...
    expect(
        ...
        errors,
        "out-of-band access validation version must be 1",
    );
    expect(
        string_field(catalog, "status") == Some("draft"),
        errors,
        "out-of-band access validation status must be draft",
    );
    expect(
        string_field(catalog, "source") == Some("static-seed"),
        errors,
        "out-of-band access validation source must be static-seed",
    );
    expect(
        string_field(catalog, "validationMode") == Some("review-only"),
        errors,
        "out-of-band access validation mode must be review-only",
    );
    for field in DISABLED_FIELDS {
        expect(
            ...
            errors,
            &format!(
                "out-of-band access validation {} must be disabled",
                field_label(field)
            ),
        );
    }
    ...
    for field in [
        ...
        for value in array_strings(catalog, field) {
            if prohibited_oob_key(&value) {
                errors.push(format!("{field} contains prohibited OOB field {value}"));
            }
        }
    }
...
fn field_label(field: &str) -> &'static str {
    match field {
        ...
        "liveAccessChecksAllowed" => "live access checks",
        "liveCertificateChecksAllowed" => "live certificate checks",
        "rawInventoryRowsAllowed" => "raw inventory rows",
        "endpointIdentifiersAllowed" => "endpoint identifiers",
        "serialNumbersAllowed" => "serial numbers",
        "accountIdentifiersAllowed" => "account identifiers",
        _ => "field",
    }
}
...
fn validate_catalog_rules(catalog: &Value, errors: &mut Vec<String>) {
    ...
    for required in REQUIRED_RULES {
        ...
        let Some(map) = rule.as_object() else {
            errors.push(format!(
                "out-of-band access validation rule {} must be a mapping",
                required.id
            ));
            continue;
        };
        ...
        if !unexpected.is_empty() {
            errors.push(format!(
                "out-of-band access validation rule {} has unexpected keys: {}",
                required.id,
                unexpected.join(", ")
            ));
        }
        ...
        expect(
            ...
            errors,
            &format!(
                "out-of-band access validation rule {} has unexpected decision",
                required.id
            ),
        );
        expect(
            ...
            errors,
            &format!(
                "out-of-band access validation rule {} has unexpected requirement",
                required.id
            ),
        );
        expect(
            ...
            errors,
            &format!(
                "out-of-band access validation rule {} has unexpected evidence",
                required.id
            ),
        );
    }
...
fn validate_docs_text(
    ...
    expect(
        ...
        errors,
        "API README missing out-of-band access validation endpoint",
    );
    expect(
        catalog_readme.contains("out-of-band-access-validation-contract.yaml"),
        errors,
        "catalog README missing out-of-band access validation catalog",
    );
    expect(
        doc_readme.contains("out-of-band-access-validation.md"),
        errors,
        "workflow README missing out-of-band access validation doc",
    );
    expect(
        doc.contains(ENDPOINT),
        errors,
        "out-of-band access validation doc missing endpoint",
    );
    expect(
        doc.contains("No live provider calls."),
        errors,
        "out-of-band access validation doc must prohibit provider calls",
    );
    expect(
        doc.contains("No live access checks."),
        errors,
        "out-of-band access validation doc must prohibit live access checks",
    );
    expect(
        doc.contains("No live certificate checks."),
        errors,
        "out-of-band access validation doc must prohibit live certificate checks",
    );
    expect(
        doc.contains("No raw inventory rows."),
        errors,
        "out-of-band access validation doc must prohibit raw inventory rows",
    );
    expect(
        doc.contains("No endpoint identifiers"),
        errors,
        "out-of-band access validation doc must prohibit endpoint identifiers",
    );
    expect(
        doc.contains("OOB-safe readiness summaries only"),
        errors,
        "out-of-band access validation doc must require safe summaries",
    );
}
...
fn endpoint_block(program: &str, errors: &mut Vec<String>) -> String {
    ...
    if starts.is_empty() {
        errors.push("API missing out-of-band access validation endpoint".to_string());
        return String::new();
    }
    if starts.len() != 1 {
        errors.push(
            "API out-of-band access validation endpoint must be declared exactly once".to_string(),
        );
        return String::new();
    }
    let start = starts[0];
...
fn endpoint_payload_block(endpoint: &str, errors: &mut Vec<String>) -> String {
    ...
    if json_indexes.is_empty() {
        errors.push("API missing out-of-band access validation JSON payload".to_string());
        return String::new();
    }
    if json_indexes.len() != 1 {
        errors.push(
            "API must declare exactly one out-of-band access validation JSON payload".to_string(),
        );
        return String::new();
    }
    ...
    let Some(object_start) = endpoint[json_index..]
        ...
        errors.push(
            "API out-of-band access validation JSON payload must be a single object".to_string(),
        );
        return String::new();
    };
...
fn validate_no_prohibited_api_terms(text: &str, label: &str, errors: &mut Vec<String>) {
    for value in quoted_values(text) {
        if prohibited_oob_key(&value) {
            errors.push(format!("{label} contains prohibited OOB field {value}"));
        }
    }
}

fn validate_no_prohibited_program_values(text: &str, label: &str, errors: &mut Vec<String>) {
    for value in quoted_values(text) {
        if contains_prohibited_value(&value) {
...
fn validate_no_prohibited_api_field_names(text: &str, label: &str, errors: &mut Vec<String>) {
    for field in assignment_field_names(text) {
        if prohibited_oob_key(&field) {
            errors.push(format!("{label} contains prohibited OOB field {field}"));
        }
    }
}
...
fn validate_endpoint_field_names(text: &str, errors: &mut Vec<String>) {
    ...
    if !unexpected.is_empty() {
        errors.push(format!(
            "API endpoint has unexpected out-of-band access validation fields: {}",
            unexpected.join(", ")
        ));
    }
}
    ...
    match value {
        Value::Object(map) => {
            for (key, child) in map {
                if prohibited_oob_key(key) {
                    errors.push(format!("{path}.{key} contains prohibited OOB field"));
                }
        ...
        Value::String(text) => {
            ...
            } else if prohibited_oob_key(text) {
                errors.push(format!("{path} contains prohibited OOB value {text}"));
            }
        }
        _ => {}
    }
...
fn validate_raw_catalog_text(text: &str, path: &str, errors: &mut Vec<String>) {
    for (index, line) in text.lines().enumerate() {
        ...
        if let Some(key) = catalog_assignment_key(line) {
            if prohibited_oob_key(&key) {
                errors.push(format!(
                    "{path}:{line_number} contains prohibited OOB field {key}"
                ));
            }
        }
        ...
        for term in words(comment_text.trim_start_matches("- ")) {
            let message = format!("{path}:{line_number} contains prohibited OOB field {term}");
            if prohibited_oob_key(&term) && !errors.contains(&message) {
                errors.push(message);
            }
        }
    }
...
fn validate_markdown_terms(text: &str, path: &str, errors: &mut Vec<String>) {
    ...
    for (index, line) in text.lines().enumerate() {
        if !oob_markdown_line_scope(path, line) || safe_text_prohibition_line(line) {
            continue;
        }
        for term in words(line) {
            let message = format!("{path}:{} contains prohibited OOB field {term}", index + 1);
            if prohibited_oob_key(&term) && !errors.contains(&message) {
                errors.push(message);
            }
        }
...
fn oob_markdown_line_scope(path: &str, line: &str) -> bool {
    path.contains(DOC_PATH)
        || line.to_ascii_lowercase().contains("out-of-band")
        || line.contains("OOB")
}

fn safe_text_prohibition_line(line: &str) -> bool {
    SAFE_TEXT_PROHIBITION_LINES.contains(&line.trim())
}
...
fn prohibited_oob_key(key: &str) -> bool {
    if safe_oob_text_value(key) {
        return false;
    }
    let normalized = normalized_key(key);
    if SAFE_OOB_GUARD_KEYS.contains(&normalized.as_str()) {
        return false;
    }
    PROHIBITED_OOB_KEYS.contains(&normalized.as_str())
        || PROHIBITED_OOB_SUBSTRINGS
            .iter()
            .any(|token| normalized.contains(token))
}

fn safe_oob_text_value(value: &str) -> bool {
    REQUIRED_CONSOLE_TYPES.contains(&value)
...
mod tests {
    ...
    #[test]
    fn out_of_band_access_endpoint_registration_detects_route_alias() {
        let program = format!(
            r#"
