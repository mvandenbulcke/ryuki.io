...
const CATALOG_PATH: &str = "catalog/operator-runbook-contract.yaml";
const PROGRAM_PATH: &str = "api/Ryuki.Platform.Api/Program.cs";
const API_README_PATH: &str = "api/Ryuki.Platform.Api/README.md";
const DOC_PATH: &str = "docs/workflows/operator-runbook.md";
const ENDPOINT: &str = "/api/operations/runbook-launch-contract";
const REQUIRED_WORKFLOWS: &[&str] = &[
    "operator-runbook-launch",
    "standard-l1-l2-task",
    "incident-context-review",
];
const REQUIRED_PLAN_TYPES: &[&str] = &[
    "health-check-plan",
    "collect-logs-plan",
...
const ENDPOINT_ARRAY_BINDINGS: &[(&str, &str)] = &[
    ("supportedWorkflows", "operatorRunbookWorkflows"),
    ("runbookPlanTypes", "operatorRunbookPlanTypes"),
    ("requiredGuards", "operatorRunbookRequiredGuards"),
    ("planSections", "operatorRunbookPlanSections"),
    ("blockedReasons", "operatorRunbookBlockedReasons"),
];
...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    ...
    let context: OperatorRunbookContext = serde_json::from_str(&payload)
        .map_err(|error| format!("invalid operator runbook context JSON: {error}"))?;
...
pub fn validate_catalog_json(input: &str) -> Result<Vec<String>, String> {
    let catalog: Value = serde_json::from_str(input)
        .map_err(|error| format!("invalid operator runbook catalog JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_program_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProgramInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid operator runbook program JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_docs_json(input: &str) -> Result<Vec<String>, String> {
    let payload: DocsInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid operator runbook docs JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn scan_prohibited_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProhibitedInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid operator runbook prohibited JSON: {error}"))?;
    let mut errors = Vec::new();
...
fn validate_catalog_value(catalog: &Value, errors: &mut Vec<String>) {
    if !catalog.is_object() {
        errors.push("operator runbook catalog must be a mapping".to_string());
        return;
    }
    ...
    expect(
        ...
        errors,
        "operator runbook version must be 1",
    );
    expect(
        string_value(catalog, "status") == Some("draft"),
        errors,
        "operator runbook status must be draft",
    );
    expect(
        string_value(catalog, "source") == Some("static-seed"),
        errors,
        "operator runbook source must be static-seed",
    );
    expect(
        bool_value(catalog, "dryRunRequired") == Some(true),
        errors,
        "operator runbook must require dry-run",
    );
    expect(
        bool_value(catalog, "providerCallsEnabled") == Some(false),
        errors,
        "operator runbook provider calls must be disabled",
    );
    expect(
        bool_value(catalog, "workerExecutionAllowed") == Some(false),
        errors,
        "operator runbook worker execution must be disabled",
    );
    validate_required_array(catalog, "supportedWorkflows", REQUIRED_WORKFLOWS, errors);
    validate_required_array(catalog, "runbookPlanTypes", REQUIRED_PLAN_TYPES, errors);
...
fn validate_catalog_field_names(catalog: &Value, errors: &mut Vec<String>) {
    ...
    for field in map.keys() {
        ...
        errors.push(format!(
            "operator runbook catalog has unexpected field {field}"
        ));
        if prohibited_provider_identifier(field) {
            errors.push(format!(
                "operator runbook catalog has prohibited field {field}"
            ));
        }
    }
    ...
    for rule in rules {
        ...
        for field in rule_map.keys() {
            ...
            errors.push(format!(
                "operator runbook rule {rule_id} has unexpected field {field}"
            ));
            if prohibited_provider_identifier(field) {
                errors.push(format!(
                    "operator runbook rule {rule_id} has prohibited field {field}"
                ));
            }
        }
    }
...
fn validate_required_rules(catalog: &Value, errors: &mut Vec<String>) {
    ...
    expect(
        ...
        errors,
        "operator runbook rule IDs must be unique",
    );
    expect(
        rule_details.iter().collect::<BTreeSet<_>>().len() == rule_details.len(),
        errors,
        "operator runbook rule details must be unique",
    );
    expect(
        missing.is_empty(),
        errors,
        format!("operator runbook missing rules: {}", missing.join(", ")),
    );
    expect(
        unexpected.is_empty(),
        errors,
        format!(
            "operator runbook unexpected rules present: {} redacted rule id(s)",
            unexpected.len()
        ),
    );
    for expected_rule in REQUIRED_RULES {
        ...
        expect(
            ...
            errors,
            format!(
                "operator runbook rule {} decision must match",
                expected_rule.id
            ),
        );
        expect(
            ...
            errors,
            format!(
                "operator runbook rule {} requirement must match",
                expected_rule.id
            ),
        );
        expect(
            ...
            errors,
            format!(
                "operator runbook rule {} evidence must match",
                expected_rule.id
            ),
        );
    }
...
fn validate_endpoint_field_names(block: &str, errors: &mut Vec<String>) {
    ...
    for field in assignment_fields(&stripped) {
        ...
        if prohibited_provider_identifier(&field) {
            errors.push(format!(
                "API endpoint has prohibited operator runbook field {field}"
            ));
        } else {
            errors.push(format!(
                "API endpoint has unexpected operator runbook field {field}"
            ));
        }
    }
}
...
fn validate_docs_text(readme: &str, doc: &str, errors: &mut Vec<String>) {
    expect(
        ...
        errors,
        "API README missing operator runbook endpoint",
    );
    expect(
        doc.contains(ENDPOINT),
        errors,
        "operator runbook doc missing endpoint",
    );
    expect(
        doc.contains("No live provider calls."),
        errors,
        "operator runbook doc must prohibit provider calls",
    );
    expect(
        doc.contains("No worker execution."),
        errors,
        "operator runbook doc must prohibit worker execution",
    );
    expect(
        doc.contains("provider-safe runbook plans"),
        errors,
        "operator runbook doc must require provider-safe plans",
    );
}
    ...
    match value {
        Value::Object(map) => {
            for (key, child) in map {
                ...
                if prohibited_provider_identifier(key) {
...
fn catalog_rules(catalog: &Value, errors: &mut Vec<String>) -> Vec<Rule> {
    let Some(Value::Array(rules)) = catalog.get("rules") else {
        errors.push("operator runbook rules must be an array of mappings".to_string());
        return Vec::new();
    };
...
fn endpoint_block(uncommented_program: &str, errors: &mut Vec<String>) -> String {
    ...
    if starts.is_empty() {
        errors.push("API missing operator runbook endpoint".to_string());
        return String::new();
    }
    expect(
        starts.len() == 1,
        errors,
        "API must expose exactly one operator runbook endpoint",
    );
...
mod tests {
    ...
    #[test]
    fn operator_runbook_response_parser_prefers_returned_json_object() {
        let call_block = r#"
app.MapGet("/api/operations/runbook-launch-contract", () =>
{
    _ = Results.Json(new
    {
        source = "static-seed",
...
"#;
        ...
        let block = endpoint_response_object_block(call_block, &mut errors)
            .expect("expected returned Results.Json response block");

        assert!(errors.is_empty());
        assert!(block.contains("source = \"live-provider\""));
