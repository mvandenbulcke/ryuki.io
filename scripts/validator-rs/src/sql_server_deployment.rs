...
const CATALOG_PATH: &str = "catalog/sql-server-deployment-contract.yaml";
const PROGRAM_PATH: &str = "api/Ryuki.Platform.Api/Program.cs";
const API_README_PATH: &str = "api/Ryuki.Platform.Api/README.md";
const CATALOG_README_PATH: &str = "catalog/README.md";
const DOC_README_PATH: &str = "docs/workflows/README.md";
const DOC_PATH: &str = "docs/workflows/sql-server-deployment.md";

const ENDPOINT: &str = "/api/workflows/sql-server/deployment-contract";
...
const REQUIRED_TOPOLOGIES: &[&str] = &["standalone", "failover-cluster", "availability-group"];
...
const REQUIRED_BLOCKED_REASONS: &[&str] = &[
    ...
    "live-dns-change-disabled",
    "live-backup-change-disabled",
    "live-monitoring-change-disabled",
    "live-cmdb-change-disabled",
    "availability-group-change-disabled",
    "sql-runtime-identity-change-disabled",
    "spn-change-disabled",
    "database-creation-disabled",
    "sql-agent-job-change-disabled",
    ...
    "sql-listener-identifiers-disabled",
    "credential-values-disabled",
    "port-values-disabled",
    "topology-missing",
    "disk-layout-missing",
...
const REQUIRED_DISABLED_FIELDS: &[&str] = &[
    ...
    "liveSqlChangesAllowed",
    "liveDirectoryChangesAllowed",
    "liveDnsChangesAllowed",
    "liveBackupChangesAllowed",
    "liveMonitoringChangesAllowed",
    "liveCmdbChangesAllowed",
    "availabilityGroupChangesAllowed",
    "sqlRuntimeIdentityChangesAllowed",
    "spnChangesAllowed",
    "databaseCreationAllowed",
    "sqlAgentJobChangesAllowed",
    "rawSqlInstanceDataAllowed",
    "rawDatabaseDataAllowed",
    "rawPathDataAllowed",
    "rawBackupRowsAllowed",
    "rawProviderPayloadsAllowed",
    "principalIdentifiersAllowed",
    "hostIdentifiersAllowed",
    "listenerIdentifiersAllowed",
...
const ENDPOINT_ARRAY_BINDINGS: &[(&str, &str, &[&str])] = &[
    ("supportedPlans", "sqlServerDeploymentPlans", REQUIRED_PLANS),
    (
        "deploymentTopologies",
        "sqlServerDeploymentTopologies",
        REQUIRED_TOPOLOGIES,
    ),
    (
        "supportedHypervisors",
        "sqlServerDeploymentSupportedHypervisors",
        REQUIRED_HYPERVISORS,
    ),
    (
        "requiredGuards",
        "sqlServerDeploymentRequiredGuards",
        REQUIRED_GUARDS,
    ),
    (
        "planSections",
        "sqlServerDeploymentPlanSections",
        REQUIRED_PLAN_SECTIONS,
    ),
    (
        "blockedReasons",
        "sqlServerDeploymentBlockedReasons",
...
const SAFE_TEXT_PROHIBITION_LINES: &[&str] = &[
    "# SQL Server deployment seed data only. Do not add SQL instance names, database names, listener names, hostnames, FQDNs, IP addresses, port values, file paths, backup paths, runtime identity names, SPN values, principal IDs, tenant IDs, object IDs, credentials, tokens, raw SQL rows, raw backup rows, raw logs, or provider payloads.",
    "- No SQL instance names, database names, listener names, hostnames, FQDNs, IP addresses, port values, file paths, backup paths, runtime identity names, SPN values, principal identifiers, tenant identifiers, object identifiers, credential values, secret values, access tokens, raw SQL rows, raw backup rows, raw logs, or provider payloads in committed files.",
    ...
    "requirement: SQL Server deployment evidence must use safe summaries only and must not expose SQL instance names, database names, listener names, hostnames, FQDNs, IP addresses, port values, file paths, backup paths, runtime identity names, SPN values, principal IDs, tenant IDs, object IDs, credentials, secret values, access tokens, raw SQL rows, raw backup rows, raw logs, or provider payloads.",
];
...
const REQUIRED_RULES: &[RuleDetail] = &[
    RuleDetail {
        ...
        decision: "block",
        requirement: "SQL Server deployment produces a dry-run plan only and never creates SQL instances, databases, availability groups, VMware, Hyper-V, or Proxmox workloads, runtime identities, Kerberos registration values, DNS records, backup jobs, monitoring objects, CMDB records, workers, or provider state.",
        evidence: "SQL deployment summary",
    },
    RuleDetail {
        id: "layout-identity-protection-required",
        decision: "block",
        requirement: "Topology, placement, disk layout, runtime identity, Kerberos registration policy, backup, monitoring, and CMDB publication plans must be reviewed before SQL Server deployment approval.",
        evidence: "Disk layout plan",
    },
    RuleDetail {
        id: "approval-rollback-required",
        decision: "block",
        requirement: "Owner, support group, approval route, rollback plan, and evidence references must be present before SQL Server deployment can be accepted.",
        evidence: "Rollback plan",
    },
    RuleDetail {
        id: "raw-sql-deployment-data-not-exposed",
        decision: "block",
        requirement: "SQL Server deployment evidence must use safe summaries only and must not expose SQL deployment identifiers, network identifiers, path values, runtime identity details, Kerberos registration details, raw SQL evidence, raw backup evidence, unredacted operations data, sensitive values, or provider-returned content.",
...
struct ContextInput {
    ...
    doc_readme: String,
    doc: String,
    // The Ruby runner/acceptance-test inputs were retired with the Ruby
    // test suite; the fields were removed so context construction no
...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    ...
    let context: ContextInput = serde_json::from_str(&payload)
        .map_err(|error| format!("invalid SQL Server deployment context JSON: {error}"))?;
    ...
    // and `{id}` path params belonging to unrelated endpoints. The SQL Server
    ...
    // source literals removed: Ruby files no longer exist
    Ok(errors)
}

pub fn validate_catalog_json(input: &str) -> Result<Vec<String>, String> {
    let catalog: Value = serde_json::from_str(input)
        .map_err(|error| format!("invalid SQL Server deployment catalog JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_program_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProgramInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid SQL Server deployment program JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_docs_json(input: &str) -> Result<Vec<String>, String> {
    let payload: DocsInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid SQL Server deployment docs JSON: {error}"))?;
    let mut errors = Vec::new();
    validate_docs_text(
        &payload.api_readme,
...
pub fn scan_prohibited_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProhibitedInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid SQL Server deployment prohibited JSON: {error}"))?;
    let mut errors = Vec::new();
    scan_prohibited_value(&payload.value, &payload.path, &mut errors);
    Ok(errors)
}

pub fn validate_source_literals_json(_input: &str) -> Result<Vec<String>, String> {
    // validate_source_literals_text removed: Ruby files no longer exist
    Ok(Vec::new())
}

fn validate_catalog_value(catalog: &Value, errors: &mut Vec<String>) {
    let Some(map) = catalog.as_object() else {
        errors.push("SQL Server deployment catalog must be a mapping".to_string());
        return;
    };
    let unexpected = map
        .keys()
    ...
    if !unexpected.is_empty() {
        errors.push(format!(
            "SQL Server deployment unexpected catalog keys: {}",
            unexpected.join(", ")
        ));
    }
    expect(
        ...
        errors,
        "SQL Server deployment version must be 1",
    );
    expect(
        value_str(catalog, "status") == Some("draft"),
        errors,
        "SQL Server deployment status must be draft",
    );
    expect(
        value_str(catalog, "source") == Some("static-seed"),
        errors,
        "SQL Server deployment source must be static-seed",
    );
    expect(
        value_str(catalog, "deploymentMode") == Some("dry-run-plan"),
        errors,
        "SQL Server deployment mode must be dry-run-plan",
    );
    expect(
        value_bool(catalog, "dryRunRequired") == Some(true),
        errors,
        "SQL Server deployment must require dry-run",
    );
    for field in REQUIRED_DISABLED_FIELDS {
        expect(
            value_bool(catalog, field) == Some(false),
            errors,
            &format!("SQL Server deployment {field} must be disabled"),
        );
    }
    validate_required_array(catalog, "supportedPlans", REQUIRED_PLANS, errors);
    validate_required_array(catalog, "deploymentTopologies", REQUIRED_TOPOLOGIES, errors);
...
fn validate_catalog_rules(catalog: &Value, errors: &mut Vec<String>) {
    ...
    if !missing.is_empty() {
        errors.push(format!(
            "SQL Server deployment missing rules: {}",
            missing.join(", ")
        ));
    }
    if !unexpected.is_empty() {
        errors.push(format!(
            "SQL Server deployment unexpected rules: {}",
            unexpected.join(", ")
        ));
    }
    expect(
        ...
        errors,
        "SQL Server deployment rule IDs must be unique",
    );
    let details = rules
        .iter()
        .filter_map(|rule| {
    ...
    expect(
        ...
        errors,
        "SQL Server deployment catalog rule details must be unique",
    );
    for expected_rule in REQUIRED_RULES {
        ...
        for (field, expected) in [
            ...
            expect(
                ...
                errors,
                &format!(
                    "SQL Server deployment rule {} {field} must match",
                    expected_rule.id
                ),
            );
        }
...
// SQL Server deployment contract is mounted as `.route(ENDPOINT, get(handler))`
...
fn validate_program_text(program: &str, _catalog: &Value, errors: &mut Vec<String>) {
    let Some(payload) = crate::rust_contract::endpoint_payload(
        program,
        ENDPOINT,
        "API missing SQL Server deployment endpoint",
        "API missing SQL Server deployment JSON payload",
        errors,
    ) else {
        return;
    };
...
fn validate_program_text_csharp(program: &str, catalog: &Value, errors: &mut Vec<String>) {
    ...
    validate_endpoint_assignment_syntax(&block, errors);
...
fn endpoint_block(program: &str, errors: &mut Vec<String>) -> String {
    ...
    if starts.is_empty() {
        errors.push("API missing SQL Server deployment endpoint".to_string());
        return String::new();
    }
    if starts.len() != 1 {
        errors.push("API SQL Server deployment endpoint must be declared exactly once".to_string());
        return String::new();
    }
    let start = starts[0];
...
fn endpoint_payload_block(endpoint: &str, errors: &mut Vec<String>) -> String {
    ...
    if json_indexes.is_empty() {
        errors.push("API missing SQL Server deployment JSON payload".to_string());
        return String::new();
    }
    if json_indexes.len() != 1 {
        errors.push("API must declare exactly one SQL Server deployment JSON payload".to_string());
        return String::new();
    }
    ...
    let Some(object_start) = endpoint[json_index..]
        ...
        errors.push("API SQL Server deployment JSON payload must be a single object".to_string());
        return String::new();
    };
...
fn validate_api_array(
    ...
    for value in values {
        ...
        if prohibited_field(value) {
            errors.push(format!(
                "API {field} contains prohibited SQL Server deployment value {value}"
            ));
        }
        if let Some(phrase) = prohibited_phrase(value) {
            errors.push(format!(
                "API {field} contains prohibited SQL Server deployment phrase {phrase}"
            ));
        }
    }
}
...
fn validate_api_rules(block: &str, catalog: &Value, errors: &mut Vec<String>) {
    ...
    expect(
        details.len() == detail_set.len(),
        errors,
        "SQL Server deployment endpoint rule details must be unique",
    );
...
fn validate_endpoint_assignment_syntax(block: &str, errors: &mut Vec<String>) {
    for line in block.lines() {
        let stripped = line.trim();
        if !stripped.contains('=') || stripped.starts_with("app.MapGet") {
            continue;
        }
        for lhs in bracket_assignment_lhs(stripped) {
            let field = csharp_string_literals(&lhs)
                .into_iter()
                .find(|literal| prohibited_field(literal))
                .unwrap_or(lhs);
            errors.push(format!(
                "API endpoint has alternate assignment syntax {field}"
            ));
        }
        if stripped.starts_with("new {") || stripped.contains("[") {
            continue;
        }
...
fn bracket_assignment_lhs(line: &str) -> Vec<String> {
    let mut results = Vec::new();
    let mut index = 0;
    while let Some(open_relative) = line[index..].find('[') {
        let open = index + open_relative;
        let Some(close_relative) = line[open..].find(']') else {
            break;
        };
        let close = open + close_relative;
        let after = skip_ascii_whitespace(line, close + 1);
        if line.as_bytes().get(after) == Some(&b'=') {
            results.push(line[open..=close].to_string());
        }
        index = close + 1;
    }
    results
    ...
    for field in top_level_assignment_fields(block) {
        if ALLOWED_ENDPOINT_FIELDS.contains(&field.as_str()) {
            if prohibited_field(&field) {
                errors.push(format!(
                    "API endpoint has prohibited SQL Server deployment field {field}"
                ));
            }
        } else {
            errors.push(format!(
                "API endpoint has unexpected SQL Server deployment field {field}"
            ));
        }
    }
}
...
fn validate_endpoint_property_identifiers(block: &str, errors: &mut Vec<String>) {
    ...
    for identifier in identifiers {
        ...
        if prohibited_field(&identifier) {
            errors.push(format!(
                "API endpoint property {identifier} contains prohibited SQL Server deployment identifier"
            ));
        }
    }
}
...
fn validate_docs_text(
    ...
    expect(
        ...
        errors,
        "API README missing SQL Server deployment endpoint",
    );
    expect(
        catalog_readme.contains(CATALOG_PATH.trim_start_matches("catalog/")),
        errors,
        "catalog README missing SQL Server deployment catalog",
    );
    expect(
        doc_readme.contains(DOC_PATH.trim_start_matches("docs/workflows/")),
        errors,
        "workflow README missing SQL Server deployment doc",
    );
    expect(
        doc.contains(ENDPOINT),
        errors,
        "SQL Server deployment doc missing endpoint",
    );
    expect(
        doc.contains("No live provider calls."),
        errors,
        "SQL Server deployment doc must prohibit provider calls",
    );
    expect(
        doc.contains("No worker execution."),
        errors,
        "SQL Server deployment doc must prohibit worker execution",
    );
    expect(
        doc.contains(
        ...
        errors,
        "SQL Server deployment doc must prohibit live provider changes",
    );
    expect(
        doc.contains("No raw SQL instance data, database data, paths, backup rows, host identifiers, listener identifiers, port values, credentials, or provider payloads."),
        errors,
        "SQL Server deployment doc must prohibit raw SQL data",
    );
    expect(
        doc.contains("static SQL Server deployment summaries only"),
        errors,
        "SQL Server deployment doc must require static summaries",
    );
    for hypervisor in REQUIRED_HYPERVISORS {
        expect(
            doc.contains(hypervisor),
            errors,
            &format!("SQL Server deployment doc missing supported hypervisor {hypervisor}"),
        );
    }
    expect(
        doc.contains("live hypervisor execution"),
        errors,
        "SQL Server deployment doc must block live hypervisor execution",
    );
}
    ...
    match value {
        Value::Object(map) => {
            for (key, child) in map {
                ...
                if prohibited_field(key) {
                    errors.push(format!(
                        "{child_path} contains prohibited SQL Server deployment field"
                    ));
                }
        ...
        Value::String(text) => {
            if whole_file_text(path, text) {
                if sql_server_deployment_text_path(path) {
            ...
            if let Some(phrase) = prohibited_phrase(text) {
                errors.push(format!(
                    "{path} contains prohibited SQL Server deployment phrase {phrase}"
                ));
            }
            if prohibited_field(text) {
                errors.push(format!(
                    "{path} contains prohibited SQL Server deployment value {text}"
                ));
            }
        }
        _ => {}
...
fn validate_text_terms(text: &str, path: &str, errors: &mut Vec<String>) {
    for (index, line) in text.lines().enumerate() {
        if !sql_server_deployment_text_line(path, line) || safe_text_line(line) {
            continue;
        }
        ...
        for term in word_terms(line) {
            if prohibited_field(&term) {
                errors.push(format!(
                    "{path}:{} contains prohibited SQL Server deployment field {term}",
                    index + 1
                ));
            }
        }
...
fn contains_prohibited_value(value: &str) -> bool {
    contains_aws_access_key(value)
        ...
        || contains_fqdn(value)
        || contains_spn_value(value)
        || contains_path_value(value)
        || port_value(value)
...
fn contains_fqdn(value: &str) -> bool {
    value
        ...
        .any(|candidate| {
            ...
            if [
                ".md", ".yaml", ".yml", ".cs", ".csproj", ".json", ".sh", ".txt", ".html", ".css",
                ".xml", ".png", ".jpg", ".jpeg", ".svg", ".pdf",
            ]
            .iter()
...
fn contains_spn_value(value: &str) -> bool {
    value.split_whitespace().any(|token| {
        ...
        let Some((service, host)) = token.split_once('/') else {
            return false;
        };
        if token.starts_with('/') || host.starts_with('/') || host.is_empty() {
            return false;
        }
        let service_lower = service.to_ascii_lowercase();
        let known = [
            "host", "http", "mssqlsvc", "termsrv", "wsman", "cifs", "ldap",
        ]
        .contains(&service_lower.as_str());
        let pathlike = [
            "api",
            "catalog",
            "deploy",
        ...
        .contains(&service_lower.as_str());
        (known || (!pathlike && is_identifier(service))) && host.chars().any(|c| c == '.')
    })
}

fn contains_path_value(value: &str) -> bool {
...
fn safe_text_line(line: &str) -> bool {
    ...
    SAFE_TEXT_PROHIBITION_LINES.contains(&stripped)
        ...
        || stripped == format!("Endpoint: `{ENDPOINT}`")
        || safe_program_endpoint_line(stripped)
        || safe_program_array_line(stripped)
}

fn safe_source_literal_for_path(_path: &str, value: &str) -> bool {
    safe_source_literal(value)
}

fn safe_source_literal(value: &str) -> bool {
    let normalized_route = value.replace("\\/", "/");
    value == ENDPOINT
        || normalized_route == ENDPOINT
        || safe_api_route_literal(&normalized_route)
        || value == "minitest/autorun"
        || safe_source_path_literal(value)
        || value.contains("#{")
}

fn safe_test_source_fragment(value: &str) -> bool {
    !value.is_empty()
        && !value.contains('/')
        && !value.contains('\\')
        && !value.contains('@')
...
fn safe_api_route_literal(value: &str) -> bool {
...
fn safe_source_path_literal(value: &str) -> bool {
    let path = value
        .strip_prefix("../")
    ...
    (path.starts_with("scripts/") && !path.rsplit('/').next().unwrap_or("").contains('.'))
        || [".md", ".yaml", ".yml", ".cs", ".txt"]
            .iter()
            .any(|extension| path.ends_with(extension))
}

fn safe_program_endpoint_line(stripped: &str) -> bool {
    stripped == format!("app.MapGet(\"{ENDPOINT}\", () => Results.Json(new")
}

fn safe_program_array_line(stripped: &str) -> bool {
    let Some((lhs, _)) = stripped.split_once('=') else {
        return false;
    };
    let field = lhs.trim().strip_prefix("var ").unwrap_or(lhs.trim()).trim();
    if !is_identifier(field) || !stripped.contains("new[]") || !stripped.contains('{') {
        return false;
    }
    let literals = csharp_string_literals(stripped);
    safe_text_value(field)
        && !literals.is_empty()
        && literals.iter().all(|literal| safe_text_value(literal))
}
...
fn safe_text_value(value: &str) -> bool {
    ...
    static_values.contains(&value)
        || REQUIRED_PLANS.contains(&value)
        || REQUIRED_TOPOLOGIES.contains(&value)
        || REQUIRED_HYPERVISORS.contains(&value)
...
fn source_prohibited_field_literal(value: &str) -> bool {
    let normalized = normalized_key(value);
    [
        "sqlInstanceName",
        "sqlInstanceIdentifier",
    ...
    .iter()
    .any(|field| normalized == normalized_key(field))
}

fn prohibited_phrase(value: &str) -> Option<&'static str> {
    let normalized = value.to_ascii_lowercase().replace(['_', '-'], " ");
    let phrases = [
        ("SQL instance name", "sql instance name"),
        ...
        ("runtime identity name", "runtime identity name"),
        ("SPN value", "spn value"),
        ("raw SQL rows", "raw sql rows"),
        ("raw backup rows", "raw backup rows"),
...
fn sql_server_deployment_text_path(path: &str) -> bool {
    [
        CATALOG_PATH,
        PROGRAM_PATH,
        DOC_PATH,
...
fn sql_server_deployment_text_line(path: &str, line: &str) -> bool {
    ...
    lower.contains("sql server")
        || lower.contains("sql-server")
        || lower.contains("sql deployment")
        || lower.contains("sqlserverdeployment")
