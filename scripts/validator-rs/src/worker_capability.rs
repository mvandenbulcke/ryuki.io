...
const CATALOG_PATH: &str = "catalog/worker-capability-contract.yaml";
const PROGRAM_PATH: &str = "api/Ryuki.Platform.Api/Program.cs";
const API_README_PATH: &str = "api/Ryuki.Platform.Api/README.md";
const DOC_PATH: &str = "docs/workflows/worker-capability-routing.md";
const ENDPOINT: &str = "/api/admin/worker-capability-contract";
...
const REQUIRED_BLOCKED_REASONS: &[&str] = &[
    "worker-unknown",
    "capability-missing",
    "identity-reference-missing",
    "protected-network-unapproved",
    "route-ambiguous",
    "approval-missing",
...
const ENDPOINT_ARRAY_BINDINGS: &[(&str, &str)] = &[
    ("capabilityTypes", "workerCapabilityTypes"),
    ("routingDimensions", "workerRoutingDimensions"),
    ("requiredGuards", "workerCapabilityRequiredGuards"),
    ("blockedReasons", "workerCapabilityBlockedReasons"),
];
...
const REQUIRED_RULES: &[RuleDetail] = &[
    RuleDetail {
        id: "no-live-worker-dispatch",
        decision: "block",
        requirement: "Worker capability routing returns metadata decisions only and never dispatches live jobs.",
        evidence: "Routing request summary",
    },
    RuleDetail {
        id: "secret-values-not-allowed",
        decision: "block",
        requirement: "Worker identity state must use secret references only and never expose credential values.",
        evidence: "Identity reference decision",
    },
    RuleDetail {
        id: "protected-network-approval-required",
        decision: "block",
        requirement: "Protected network routes require explicit approval and network-zone decision evidence.",
        evidence: "Network zone decision",
    },
    RuleDetail {
        id: "ambiguous-route-blocks-dispatch",
        decision: "block",
        requirement: "Ambiguous or missing capability matches block dispatch until reviewed.",
        evidence: "Capability match",
    },
];
...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    ...
    let context: Context = serde_json::from_str(&payload)
        .map_err(|error| format!("invalid worker capability context JSON: {error}"))?;
    ...
    // and `{id}` path params belonging to unrelated endpoints. The
    // worker-capability handler payload is scanned for live safety flags in
    ...
    validate_no_prohibited_values(&scope, "worker-capability", &mut errors);
    Ok(errors)
}

pub fn validate_catalog_json(input: &str) -> Result<Vec<String>, String> {
    let catalog: Value = serde_json::from_str(input)
        .map_err(|error| format!("invalid worker capability catalog JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_program_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProgramInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid worker capability program JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_docs_json(input: &str) -> Result<Vec<String>, String> {
    let payload: DocsInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid worker capability docs JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn scan_prohibited_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProhibitedInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid worker capability prohibited JSON: {error}"))?;
    let mut errors = Vec::new();
...
fn validate_catalog_value(catalog: &Value, errors: &mut Vec<String>) {
    expect(
        ...
        errors,
        "worker capability version must be 1",
    );
    expect(
        catalog.get("status").and_then(Value::as_str) == Some("draft"),
        errors,
        "worker capability status must be draft",
    );
    expect(
        catalog.get("routingMode").and_then(Value::as_str) == Some("metadata-only"),
        errors,
        "worker capability routing mode must be metadata-only",
    );
    for field in [
        "providerCallsEnabled",
        "liveDispatchAllowed",
        ...
        expect(
            ...
            errors,
            format!("worker capability {field} must be disabled"),
        );
    }
    validate_required_array(catalog, "capabilityTypes", REQUIRED_CAPABILITIES, errors);
    validate_required_array(catalog, "routingDimensions", REQUIRED_DIMENSIONS, errors);
...
fn validate_required_rules(catalog: &Value, errors: &mut Vec<String>) {
    let Some(rules) = catalog.get("rules").and_then(Value::as_array) else {
        errors.push("worker capability rules must be an array of hashes".to_string());
        return;
    };
    ...
    expect(
        ...
        errors,
        "worker capability rule IDs must be unique",
    );
    ...
    expect(
        ...
        errors,
        "worker capability rule details must be unique",
    );
    expect(
        missing.is_empty(),
        errors,
        format!("worker capability missing rules: {}", missing.join(", ")),
    );
    for expected_rule in REQUIRED_RULES {
        ...
        for (field, expected) in [
            ...
            expect(
                ...
                errors,
                format!(
                    "worker capability rule {} {field} must match",
                    expected_rule.id
                ),
            );
        }
...
// worker-capability contract is mounted as `.route(ENDPOINT, get(handler))` and
...
fn validate_program_text(program: &str, _catalog: &Value, errors: &mut Vec<String>) {
    let Some(payload) = crate::rust_contract::endpoint_payload(
        program,
        ENDPOINT,
        "API missing worker capability endpoint",
        "API missing worker capability JSON payload",
        errors,
    ) else {
        return;
    };
...
fn validate_program_text_csharp(program: &str, catalog: &Value, errors: &mut Vec<String>) {
    ...
    validate_exact_string_assignment(
        ...
        "metadata-only",
        errors,
        "API must keep metadata-only routing mode",
    );
    for field in [
        "providerCallsEnabled",
        "liveDispatchAllowed",
...
fn validate_docs_text(readme: &str, doc: &str, errors: &mut Vec<String>) {
    expect(
        ...
        errors,
        "API README missing worker capability endpoint",
    );
    expect(
        doc.contains(ENDPOINT),
        errors,
        "worker capability doc missing endpoint",
    );
    expect(
        doc.contains("No live provider calls."),
        errors,
        "worker capability doc must prohibit provider calls",
    );
    expect(
        doc.contains("No live worker dispatch."),
        errors,
        "worker capability doc must prohibit worker dispatch",
    );
    expect(
        doc.contains("No secret values"),
        errors,
        "worker capability doc must prohibit secret values",
    );
}
...
fn endpoint_block(program: &str, errors: &mut Vec<String>) -> String {
    ...
    if matching_routes.is_empty() {
        errors.push("API missing worker capability endpoint".to_string());
        return String::new();
    };
    if matching_routes.len() != 1 {
        errors
            .push("API worker capability endpoint must have exactly one active route".to_string());
        return String::new();
    }
    ...
    if registration.method != "MapGet" || registration.route != ENDPOINT {
        errors.push(
            "API worker capability endpoint route must be exact canonical MapGet".to_string(),
        );
        return String::new();
    }
    ...
    if !call_tail.trim_start().starts_with(';') {
        errors.push(
            "API worker capability endpoint must not use endpoint builder chains".to_string(),
        );
        return String::new();
    }
        ...
        if let Some(mut route) = route {
            if receiver_style {
                if let Some(prefix) = group_prefix_before(program, method_start, &aliases) {
                    if route != "<nonliteral-worker-route>" {
            ...
            if route != ENDPOINT
                ...
                && contains_worker_route_terms(&route)
            {
                route = "<nonliteral-worker-route>".to_string();
            }
            if route == ENDPOINT
                || normalize_route(&route) == normalize_route(ENDPOINT)
                || route == "<nonliteral-worker-route>"
            {
    ...
    aliases
        ...
        .filter(|(_key, value)| {
            value == ENDPOINT
                ...
                || value == "<nonliteral-worker-route>"
                || contains_worker_route_terms(value)
        })
        .collect()
}
...
fn route_expression_value(expr: &str, aliases: &HashMap<String, String>) -> Option<String> {
    ...
    for part in parts {
        ...
        if let Some((identifier, _start, end)) = parse_identifier_at(trimmed, 0) {
            if !trimmed[end..].trim().is_empty() {
                return contains_worker_route_terms(trimmed)
                    .then_some("<nonliteral-worker-route>".to_string());
            }
...
fn contains_worker_route_terms(call: &str) -> bool {
    [
        "workerCapability",
        "WorkerCapability",
        "liveDispatchAllowed",
...
fn validate_endpoint_field_names(block: &str, errors: &mut Vec<String>) {
    ...
    for field in endpoint_assignment_fields(&block_without_rules) {
        if RULE_KEYS.contains(&field.as_str()) {
            errors.push(format!(
                "API endpoint has rule field outside worker capability rules {field}"
            ));
            continue;
        }
        ...
        if prohibited_endpoint_field(&field) {
            errors.push(format!(
                "API endpoint has prohibited worker capability field {field}"
            ));
        } else {
            errors.push(format!(
                "API endpoint has unexpected worker capability field {field}"
            ));
        }
    }
}
...
fn validate_endpoint_string_literals(block: &str, errors: &mut Vec<String>) {
    ...
    for literal in csharp_string_literals(block) {
        ...
        if prohibited_endpoint_field(&literal) {
            errors.push(format!(
                "API endpoint contains prohibited worker capability literal {literal}"
            ));
        }
    }
}
...
fn safe_endpoint_literals() -> HashSet<&'static str> {
    ...
    values.extend(REQUIRED_CAPABILITIES.iter().copied());
    values.extend(REQUIRED_DIMENSIONS.iter().copied());
    ...
    values.extend(["static-seed", "metadata-only", "block"]);
    values
}
...
mod tests {
    ...
    fn valid_catalog() -> Value {
        json!({
            "version": 1,
            "status": "draft",
            "routingMode": "metadata-only",
            "providerCallsEnabled": false,
            "liveDispatchAllowed": false,
            "secretValuesAllowed": false,
            "capabilityTypes": ["generic-worker", "windows-gmsa", "powercli", "linux-ansible", "protected-network"],
            "routingDimensions": ["workflowType", "osFamily", "site", "networkZone", "providerDomain", "riskLevel", "approvalState"],
            "requiredInputs": ["workflowType", "requestedCapability", "site", "environment", "networkZone", "riskLevel", "approvalRoute", "evidenceManifest"],
            "requiredGuards": ["worker-registered", "capability-tag-known", "identity-reference-ready", "network-zone-approved", "approval-route-assigned", "dry-run-ready", "evidence-redacted"],
            "blockedReasons": ["worker-unknown", "capability-missing", "identity-reference-missing", "protected-network-unapproved", "route-ambiguous", "approval-missing", "worker-health-unknown", "evidence-not-redacted"],
            "requiredEvidence": ["Routing request summary", "Capability match", "Worker readiness", "Identity reference decision", "Network zone decision", "Approval route", "Dry-run readiness", "Evidence references"],
            "rules": [
                {"id": "no-live-worker-dispatch", "decision": "block", "requirement": "Worker capability routing returns metadata decisions only and never dispatches live jobs.", "evidence": "Routing request summary"},
                {"id": "secret-values-not-allowed", "decision": "block", "requirement": "Worker identity state must use secret references only and never expose credential values.", "evidence": "Identity reference decision"},
                {"id": "protected-network-approval-required", "decision": "block", "requirement": "Protected network routes require explicit approval and network-zone decision evidence.", "evidence": "Network zone decision"},
                {"id": "ambiguous-route-blocks-dispatch", "decision": "block", "requirement": "Ambiguous or missing capability matches block dispatch until reviewed.", "evidence": "Capability match"}
            ]
        })
    }

    fn valid_program() -> String {
        r#"var workerCapabilityTypes = new[] { "generic-worker", "windows-gmsa", "powercli", "linux-ansible", "protected-network" };
var workerRoutingDimensions = new[] { "workflowType", "osFamily", "site", "networkZone", "providerDomain", "riskLevel", "approvalState" };
var workerCapabilityRequiredGuards = new[] { "worker-registered", "capability-tag-known", "identity-reference-ready", "network-zone-approved", "approval-route-assigned", "dry-run-ready", "evidence-redacted" };
var workerCapabilityBlockedReasons = new[] { "worker-unknown", "capability-missing", "identity-reference-missing", "protected-network-unapproved", "route-ambiguous", "approval-missing", "worker-health-unknown", "evidence-not-redacted" };
app.MapGet("/api/admin/worker-capability-contract", () => Results.Json(new
{
    source = "static-seed",
    routingMode = "metadata-only",
    providerCallsEnabled = false,
    ...
    requiredGuards = workerCapabilityRequiredGuards,
    blockedReasons = workerCapabilityBlockedReasons,
    requiredInputs = new[] { "workflowType", "requestedCapability", "site", "environment", "networkZone", "riskLevel", "approvalRoute", "evidenceManifest" },
    requiredEvidence = new[] { "Routing request summary", "Capability match", "Worker readiness", "Identity reference decision", "Network zone decision", "Approval route", "Dry-run readiness", "Evidence references" },
    rules = new[]
    {
        new { id = "no-live-worker-dispatch", decision = "block", requirement = "Worker capability routing returns metadata decisions only and never dispatches live jobs.", evidence = "Routing request summary" },
        new { id = "secret-values-not-allowed", decision = "block", requirement = "Worker identity state must use secret references only and never expose credential values.", evidence = "Identity reference decision" },
        new { id = "protected-network-approval-required", decision = "block", requirement = "Protected network routes require explicit approval and network-zone decision evidence.", evidence = "Network zone decision" },
        new { id = "ambiguous-route-blocks-dispatch", decision = "block", requirement = "Ambiguous or missing capability matches block dispatch until reviewed.", evidence = "Capability match" }
    }
}));
app.MapGet("/api/admin/other-contract", () => Results.Ok());"#
            .to_string()
    }

    fn program_errors(program: &str) -> Vec<String> {
        let mut errors = Vec::new();
        // These fixtures exercise the retained C# Program.cs parser; the live
        // path now uses the Rust route check in `validate_program_text`.
        validate_program_text_csharp(program, &valid_catalog(), &mut errors);
        errors
    }

    #[test]
    fn worker_capability_comment_decoy_does_not_satisfy_endpoint_contract() {
        let program = valid_program().replace(
            "    routingMode = \"metadata-only\",\n",
            "    // routingMode = \"metadata-only\",\n    routingMode = \"live-dispatch\",\n",
        );
        ...
        assert!(errors
            .iter()
            .any(|error| error.contains("metadata-only routing mode")));
    }

    #[test]
    fn worker_capability_string_decoy_route_does_not_create_duplicate_endpoint() {
        let program = format!(
            "{}\nvar routeDecoy = \"app.MapGet(\\\"/api/admin/worker-capability-contract\\\")\";",
            valid_program()
        );
        ...
        assert!(!errors
            .iter()
            .any(|error| error.contains("exactly one active route")));
    }

    #[test]
    fn worker_capability_duplicate_endpoint_and_rule_details_are_rejected() {
        let duplicate_endpoint = format!(
            "{}\napp   .   MapGet(\"/api/admin/worker-capability-contract\", () => Results.Text(\"shadow\"));",
            valid_program()
        );
        let duplicate_rule_details = valid_program().replace(
            r#"new { id = "ambiguous-route-blocks-dispatch", decision = "block", requirement = "Ambiguous or missing capability matches block dispatch until reviewed.", evidence = "Capability match" }"#,
            r#"new { id = "ambiguous-route-blocks-dispatch", decision = "block", requirement = "Worker identity state must use secret references only and never expose credential values.", evidence = "Identity reference decision" }"#,
        );

        let endpoint_errors = program_errors(&duplicate_endpoint);
        let rule_errors = program_errors(&duplicate_rule_details);
    ...
    #[test]
    fn worker_capability_broad_suffix_true_bypass_is_rejected() {
        let program = valid_program().replace(
            "    source = \"static-seed\",\n",
            "    source = \"static-seed\",\n    dryRunBypassAllowed = true,\n",
        );

        let errors = program_errors(&program);

        assert!(errors.iter().any(
            |error| error.contains("unsafe true flag") && error.contains("dryRunBypassAllowed")
        ));
    }

    #[test]
    fn worker_capability_prohibited_scan_checks_quoted_values_only() {
        let mut key_only_errors = Vec::new();
        let mut value_errors = Vec::new();

        validate_no_prohibited_values(
            &json!({ "tenantId": "summary" }),
            "root",
            &mut key_only_errors,
        );
        validate_no_prohibited_values(
            &json!({ "field": "https://example.invalid/path" }),
            "root",
            &mut value_errors,
        );

        assert!(key_only_errors.is_empty());
        assert!(value_errors
            .iter()
            .any(|error| error.contains("root.field contains prohibited value")));
    }

    #[test]
    fn worker_capability_endpoint_property_identifiers_are_rejected() {
        let host_program = valid_program().replace(
            "    source = \"static-seed\",\n",
            "    source = \"static-seed\",\n    hostName = \"summary\",\n",
        );
        let tenant_program = valid_program().replace(
            "    source = \"static-seed\",\n",
            "    source = \"static-seed\",\n    _tenantId = \"summary\",\n",
        );

        let host_errors = program_errors(&host_program);
        let tenant_errors = program_errors(&tenant_program);

        assert!(host_errors.iter().any(|error| error.contains("hostName")));
        assert!(tenant_errors
            .iter()
            .any(|error| error.contains("_tenantId")));
    }

    #[test]
    fn worker_capability_unsafe_worker_provider_literals_are_rejected() {
        let program = valid_program().replace(
            r#"requiredInputs = new[] { "workflowType", "requestedCapability", "site", "environment", "networkZone", "riskLevel", "approvalRoute", "evidenceManifest" }"#,
            r#"requiredInputs = new[] { "workflowType", "requestedCapability", "tenant_id", "site", "environment", "networkZone", "riskLevel", "approvalRoute", "evidenceManifest" }"#,
        );

        let errors = program_errors(&program);

        assert!(errors.iter().any(|error| {
            error.contains("prohibited worker capability literal") && error.contains("tenant_id")
        }));
    }
}
