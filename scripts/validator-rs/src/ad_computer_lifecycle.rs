...
const CATALOG_PATH: &str = "catalog/ad-computer-lifecycle-contract.yaml";
const API_README_PATH: &str = "api/Ryuki.Platform.Api/README.md";
const CATALOG_README_PATH: &str = "catalog/README.md";
const DOC_README_PATH: &str = "docs/workflows/README.md";
const DOC_PATH: &str = "docs/workflows/ad-computer-lifecycle.md";
const ENDPOINT: &str = "/api/identity/ad-computer-lifecycle-contract";

const REQUIRED_ACTIONS: &[&str] = &[
    "computer-prestage-review",
    "computer-move-review",
    "computer-disable-review",
    "computer-delete-review",
    "computer-recover-review",
    "computer-reconcile-review",
];
const REQUIRED_SIGNALS: &[&str] = &[
    "ou-policy-match",
    "lifecycle-state-match",
    "cmdb-state-match",
    "approval-state",
    "rollback-window-ready",
    "stale-object-risk",
    "evidence-redaction",
];
...
const REQUIRED_BLOCKED_REASONS: &[&str] = &[
    ...
    "computer-prestage-disabled",
    "computer-move-disabled",
...
const REQUIRED_DISABLED_FIELDS: &[&str] = &[
    ...
    "computerPrestageAllowed",
    "computerMoveAllowed",
    "computerDisableAllowed",
    "computerDeleteAllowed",
    "computerRecoverAllowed",
    "rawComputerDataAllowed",
...
const ENDPOINT_ARRAY_BINDINGS: &[(&str, &str, &[&str])] = &[
    (
        "lifecycleActions",
        "adComputerLifecycleActions",
        REQUIRED_ACTIONS,
    ),
    (
        "reviewSignals",
        "adComputerLifecycleSignals",
        REQUIRED_SIGNALS,
    ),
    (
        "requiredGuards",
        "adComputerLifecycleRequiredGuards",
        REQUIRED_GUARDS,
    ),
    (
        "planSections",
        "adComputerLifecyclePlanSections",
        REQUIRED_PLAN_SECTIONS,
    ),
    (
        "blockedReasons",
        "adComputerLifecycleBlockedReasons",
...
const REQUIRED_RULE_DETAILS: &[(&str, &str, &str, &str)] = &[
    ...
    (
        ...
        "block",
        "OU policy, lifecycle state, CMDB state, owner, and support group must be reviewed before any lifecycle decision can be accepted.",
        "OU policy review",
    ),
    (
        "approval-and-rollback-required",
        "block",
        "Approval route, rollback plan, and recovery readiness must be present before future directory execution can be considered.",
        "Rollback plan",
    ),
    (
        "raw-ad-computer-data-not-exposed",
        "block",
        "AD computer lifecycle evidence must use safe summaries only and must not expose computer names, hostnames, FQDNs, sAMAccountNames, distinguished names, canonical names, domain names, OU distinguished names, service principal names, object GUIDs, object SIDs, security identifiers, principal IDs, user names, group names, tenant IDs, object IDs, private IPs, credentials, secret values, access tokens, raw computer data, or provider payloads.",
        "Evidence references",
    ),
];
const SAFE_TEXT_PROHIBITION_LINES: &[&str] = &[
    "# AD computer lifecycle seed data only. Do not add computer names, hostnames, FQDNs, sAMAccountNames, distinguished names, canonical names, domain names, OU distinguished names, service principal names, object GUIDs, object SIDs, security identifiers, principal IDs, user names, group names, tenant IDs, object IDs, live endpoints, private IPs, credentials, tokens, raw computer data, or provider payloads.",
    "- No computer names, hostnames, FQDNs, sAMAccountNames, distinguished names, canonical names, domain names, OU distinguished names, service principal names, object GUIDs, object SIDs, security identifiers, principal identifiers, user names, group names, tenant identifiers, object identifiers, private network details, credential values, secret values, access tokens, raw computer data, or provider payloads in committed files.",
    "| `/api/identity/ad-computer-lifecycle-contract` | Static AD computer lifecycle review contract; live directory changes and raw AD identifiers disabled. |",
    "requirement: AD computer lifecycle evidence must use safe summaries only and must not expose computer names, hostnames, FQDNs, sAMAccountNames, distinguished names, canonical names, domain names, OU distinguished names, service principal names, object GUIDs, object SIDs, security identifiers, principal IDs, user names, group names, tenant IDs, object IDs, private IPs, credentials, secret values, access tokens, raw computer data, or provider payloads.",
];
...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    ...
    let context: ValidationContext = serde_json::from_str(&input)
        .map_err(|error| format!("invalid AD computer lifecycle context JSON: {error}"))?;
    ...
    validate_no_prohibited_values(&docs, "ad-computer-lifecycle", &mut errors);
    Ok(errors)
}

pub fn validate_catalog_json(input: &str) -> Result<Vec<String>, String> {
    let catalog: Value = serde_json::from_str(input)
        .map_err(|error| format!("invalid AD computer lifecycle catalog JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_program_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProgramInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid AD computer lifecycle program JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_docs_json(input: &str) -> Result<Vec<String>, String> {
    let payload: DocsInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid AD computer lifecycle docs JSON: {error}"))?;
    let mut errors = Vec::new();
    validate_docs_values(
        &payload.api_readme,
...
pub fn scan_prohibited_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ScanInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid AD computer lifecycle scan JSON: {error}"))?;
    let mut errors = Vec::new();
...
fn validate_catalog_value(catalog: &Value, errors: &mut Vec<String>) {
    let Some(object) = catalog.as_object() else {
        errors.push("AD computer lifecycle catalog must be an object".to_string());
        return;
    };
    ...
    if !unexpected.is_empty() {
        errors.push(format!(
            "AD computer lifecycle unexpected catalog keys: {}",
            unexpected.join(", ")
        ));
    }
    ...
    expect(
        ...
        errors,
        "AD computer lifecycle version must be 1",
    );
    expect(
        string_field(catalog, "status") == Some("draft"),
        errors,
        "AD computer lifecycle status must be draft",
    );
    expect(
        string_field(catalog, "source") == Some("static-seed"),
        errors,
        "AD computer lifecycle source must be static-seed",
    );
    expect(
        string_field(catalog, "lifecycleMode") == Some("review-only"),
        errors,
        "AD computer lifecycle mode must be review-only",
    );
    expect(
        bool_field(catalog, "dryRunRequired") == Some(true),
        errors,
        "AD computer lifecycle must require dry-run",
    );
    for field in REQUIRED_DISABLED_FIELDS {
        expect(
            bool_field(catalog, field) == Some(false),
            errors,
            &format!("AD computer lifecycle {field} must be disabled"),
        );
    }

    validate_required_array(catalog, "lifecycleActions", REQUIRED_ACTIONS, errors);
...
fn validate_required_array(
    ...
    for value in values {
        ...
        if prohibited_field(&value) {
            errors.push(format!(
                "{field} contains prohibited AD computer lifecycle value {value}"
            ));
        }
        if let Some(phrase) = prohibited_phrase(&value) {
            errors.push(format!(
                "{field} contains prohibited AD computer lifecycle phrase {phrase}"
            ));
        }
    }
}
...
fn validate_required_rules(catalog: &Value, errors: &mut Vec<String>) {
    ...
    let Some(rules) = rules else {
        errors.push("AD computer lifecycle missing rules".to_string());
        return;
    };
    ...
    if !missing.is_empty() {
        errors.push(format!(
            "AD computer lifecycle missing rules: {}",
            missing.join(", ")
        ));
    }
    if !unexpected.is_empty() {
        errors.push(format!(
            "AD computer lifecycle unexpected rules: {}",
            unexpected.join(", ")
        ));
    }
    expect(
        ...
        errors,
        "AD computer lifecycle rule IDs must be unique",
    );
    validate_rule_detail_uniqueness(&catalog_rules(catalog), "AD computer lifecycle", errors);
    for (id, decision, requirement, evidence) in REQUIRED_RULE_DETAILS {
        ...
        expect(
            ...
            errors,
            &format!("AD computer lifecycle rule {id} decision must match"),
        );
        expect(
            string_field(rule, "requirement") == Some(*requirement),
            errors,
            &format!("AD computer lifecycle rule {id} requirement must match"),
        );
        expect(
            string_field(rule, "evidence") == Some(*evidence),
            errors,
            &format!("AD computer lifecycle rule {id} evidence must match"),
        );
    }
}
...
fn validate_api_array(
    ...
    for value in values {
        ...
        if prohibited_field(&value) {
            errors.push(format!(
                "API {field} contains prohibited AD computer lifecycle value {value}"
            ));
        }
        if let Some(phrase) = prohibited_phrase(&value) {
            errors.push(format!(
                "API {field} contains prohibited AD computer lifecycle phrase {phrase}"
            ));
        }
    }
}
...
fn validate_endpoint_field_names(block: &str, errors: &mut Vec<String>) {
    for field in assignment_fields(block) {
        if !ALLOWED_ENDPOINT_FIELDS.contains(&field.as_str()) {
            errors.push(format!(
                "API endpoint has unexpected AD computer lifecycle field {field}"
            ));
            continue;
        }
        if prohibited_field(&field) {
            errors.push(format!(
                "API endpoint has prohibited AD computer lifecycle field {field}"
            ));
        }
    }
}
...
fn validate_docs_values(
    ...
    expect(
        ...
        errors,
        "API README missing AD computer lifecycle endpoint",
    );
    expect(
        catalog_readme.contains("ad-computer-lifecycle-contract.yaml"),
        errors,
        "catalog README missing AD computer lifecycle catalog",
    );
    expect(
        doc_readme.contains("ad-computer-lifecycle.md"),
        errors,
        "workflow README missing AD computer lifecycle doc",
    );
    expect(
        doc.contains(ENDPOINT),
        errors,
        "AD computer lifecycle doc missing endpoint",
    );
    expect(
        doc.contains("No live provider calls."),
        errors,
        "AD computer lifecycle doc must prohibit provider calls",
    );
    expect(
        doc.contains("No worker execution."),
        errors,
        "AD computer lifecycle doc must prohibit worker execution",
    );
    expect(
        doc.contains("No live directory changes."),
        errors,
        "AD computer lifecycle doc must prohibit directory changes",
    );
    expect(
        doc.contains("No computer prestage, move, disable, delete, or recover actions."),
        errors,
        "AD computer lifecycle doc must prohibit computer actions",
    );
    expect(
        doc.contains("static AD computer lifecycle summaries only"),
        errors,
        "AD computer lifecycle doc must require static summaries",
    );
}
    ...
    match value {
        Value::Object(object) => {
            for (key, child) in object {
                if prohibited_field(key) {
                    errors.push(format!(
                        "{path}.{key} contains prohibited AD computer lifecycle field"
                    ));
                }
        ...
        Value::String(text) => {
            ...
            if let Some(phrase) = prohibited_phrase(text) {
                errors.push(format!(
                    "{path} contains prohibited AD computer lifecycle phrase {phrase}"
                ));
            }
            if prohibited_field(text) {
                errors.push(format!(
                    "{path} contains prohibited AD computer lifecycle value {text}"
                ));
            }
        }
        _ => {}
    ...
    for (index, line) in text.lines().enumerate() {
        ...
        for term in text_terms(line) {
            if prohibited_field(&term) {
                errors.push(format!(
                    "{path}:{} contains prohibited AD computer lifecycle field {term}",
                    index + 1
                ));
            }
        }
...
fn validate_endpoint_payload_shape(block: &str, errors: &mut Vec<String>) {
    let indexes = results_json_new_indexes(block);
    if indexes.is_empty() {
        errors.push("API missing AD computer lifecycle JSON payload".to_string());
        return;
    }
    if indexes.len() != 1 {
        errors.push("API must declare exactly one AD computer lifecycle JSON payload".to_string());
        return;
    }
    let new_end = indexes[0];
    let masked = csharp_code_mask(block);
    let object_start = skip_ascii_whitespace(&masked, new_end);
    if object_start >= masked.len() || masked.as_bytes()[object_start] != b'{' {
        errors.push("API AD computer lifecycle JSON payload must be a single object".to_string());
        return;
    }
...
fn results_json_new_indexes(block: &str) -> Vec<usize> {
    ...
    while let Some(results_start) = identifier_match(&masked, "Results", index) {
        ...
        if identifier_at(&masked, new_start, "new") {
            indexes.push(new_start + "new".len());
        }
        index = results_start + "Results".len();
    }
    indexes
...
fn endpoint_rules_array_block<'a>(block: &'a str, errors: &mut Vec<String>) -> Option<&'a str> {
    ...
    if !starts_with_new_array(&masked, after_equals) {
        errors.push("API rules array must be a single literal array".to_string());
        return None;
    }
    let Some(open_offset) = masked[after_equals..].find('{') else {
        errors.push("API rules array must be a single literal array".to_string());
...
fn prohibited_phrase(value: &str) -> Option<&'static str> {
    ...
    if has_adjacent(&tokens, "computer", &["name", "names"]) {
        return Some("computer name");
    }
    ...
    if has_adjacent(&tokens, "distinguished", &["name", "names"]) {
        return Some("distinguished name");
    }
    if has_adjacent(&tokens, "canonical", &["name", "names"]) {
        return Some("canonical name");
    }
    if has_adjacent(&tokens, "domain", &["name", "names"]) {
        return Some("domain name");
    }
    if tokens.windows(3).any(|window| {
        window[0] == "ou"
            && window[1] == "distinguished"
            && matches!(window[2].as_str(), "name" | "names")
    }) {
        return Some("OU distinguished name");
    }
    if tokens.windows(3).any(|window| {
        window[0] == "service"
            && window[1] == "principal"
        ...
        return Some("service principal name");
    }
    if has_adjacent(&tokens, "object", &["guid", "guids"]) {
        return Some("object GUID");
    }
    if has_adjacent(&tokens, "object", &["sid", "sids"]) {
        return Some("object SID");
    }
    if has_adjacent(&tokens, "security", &["identifier", "identifiers"]) {
        return Some("security identifier");
    }
    ...
    if tokens
        .windows(3)
        .any(|window| window[0] == "raw" && window[1] == "computer" && window[2] == "data")
    {
        return Some("raw computer data");
    }
...
fn entra_readiness_text_line(path: &str, line: &str) -> bool {
    ...
    lower.contains("ad computer")
        || lower.contains("ad-computer")
        || lower.contains("directory computer")
