...
const REQUIRED_WORKFLOWS: &[&str] = &[
    "windows-event-forwarding-readiness",
    "linux-rsyslog-readiness",
    "linux-auditd-readiness",
    "siem-routing-review",
    "agent-policy-review",
    "evidence-pack-review",
];
const REQUIRED_SIGNALS: &[&str] = &[
    "missing-log-forwarder",
    "unsupported-agent",
    "policy-mismatch",
    "stale-log-source",
    "routing-missing",
...
const REQUIRED_BLOCKED_REASONS: &[&str] = &[
    ...
    "live-agent-install-disabled",
    "live-config-change-disabled",
    "log-platform-mutation-disabled",
    "unsupported-os-family",
    "log-profile-missing",
    "forwarding-policy-missing",
    "owner-unknown",
...
const REQUIRED_RULES: &[(&str, &str, &str, &str)] = &[
    (
        "no-live-log-forwarder-changes",
        "block",
        "Log forwarder onboarding produces readiness and dry-run plans only, never installing agents, changing collectors, or mutating log platform routes.",
        "Remediation dry-run plan",
    ),
    (
        "log-profile-required",
        "block",
        "Log forwarding decisions require a known log profile for the workload and operating system family.",
        "Forwarding policy",
    ),
    (
        "route-review-required",
        "block",
        "SIEM or operations log routing must be reviewed before onboarding can pass.",
        "Route review",
    ),
    (
        "dry-run-installation-plan-required",
        "block",
        "Any missing forwarder requires a dry-run installation or configuration plan before approval.",
        "Remediation dry-run plan",
    ),
    (
        "raw-log-samples-not-exposed",
        "block",
        "Operators receive log onboarding summaries only, not raw log samples, collector identifiers, or provider output.",
        "Onboarding summary",
    ),
];
const REQUIRED_DISABLED_FIELDS: &[&str] = &[
    "providerCallsEnabled",
    "liveAgentInstallAllowed",
    "liveConfigChangeAllowed",
    "logPlatformMutationAllowed",
    "rawLogSamplesAllowed",
];
const ENDPOINT_ARRAY_BINDINGS: &[(&str, &str, &[&str])] = &[
    (
        "supportedWorkflows",
        "logForwarderWorkflows",
        REQUIRED_WORKFLOWS,
    ),
    ("logSignals", "logForwarderSignals", REQUIRED_SIGNALS),
    (
        "requiredGuards",
        "logForwarderRequiredGuards",
        REQUIRED_GUARDS,
    ),
    (
        "planSections",
        "logForwarderPlanSections",
        REQUIRED_PLAN_SECTIONS,
    ),
    (
        "blockedReasons",
        "logForwarderBlockedReasons",
...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    ...
    let context: ContextInput = serde_json::from_str(&payload)
        .map_err(|error| format!("invalid log forwarder onboarding context JSON: {error}"))?;
...
pub fn validate_catalog_json(input: &str) -> Result<Vec<String>, String> {
    let catalog: Value = serde_json::from_str(input)
        .map_err(|error| format!("invalid log forwarder onboarding catalog JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_program_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProgramInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid log forwarder onboarding program JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_docs_json(input: &str) -> Result<Vec<String>, String> {
    let payload: DocsInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid log forwarder onboarding docs JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn scan_prohibited_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProhibitedInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid log forwarder onboarding prohibited JSON: {error}"))?;
    let mut errors = Vec::new();
...
fn validate_catalog_value(catalog: &Value, errors: &mut Vec<String>) {
    if !catalog.is_object() {
        errors.push("log forwarder onboarding catalog must be a mapping".to_string());
        return;
    }
    expect(
        value_i64(catalog, "version") == Some(1),
        errors,
        "log forwarder onboarding version must be 1",
    );
    expect(
        value_str(catalog, "status") == Some("draft"),
        errors,
        "log forwarder onboarding status must be draft",
    );
    expect(
        value_str(catalog, "source") == Some("static-seed"),
        errors,
        "log forwarder onboarding catalog must keep static-seed source",
    );
    expect(
        value_str(catalog, "onboardingMode") == Some("dry-run-readiness"),
        errors,
        "log forwarder onboarding mode must be dry-run-readiness",
    );
    expect(
        value_bool(catalog, "dryRunRequired") == Some(true),
        errors,
        "log forwarder onboarding must require dry-run",
    );
    for field in REQUIRED_DISABLED_FIELDS {
        expect(
            value_bool(catalog, field) == Some(false),
            errors,
            &format!("log forwarder onboarding {field} must be disabled"),
        );
    }
    validate_required_array(catalog, "supportedWorkflows", REQUIRED_WORKFLOWS, errors);
    validate_required_array(catalog, "logSignals", REQUIRED_SIGNALS, errors);
...
fn validate_catalog_rules(catalog: &Value, errors: &mut Vec<String>) {
    ...
    expect(
        ...
        errors,
        "log forwarder onboarding rule IDs must be unique",
    );
    if !missing.is_empty() {
        errors.push(format!(
            "log forwarder onboarding missing rules: {}",
            missing.join(", ")
        ));
    }
    if !unexpected.is_empty() {
        errors.push(format!(
            "log forwarder onboarding unexpected rules: {}",
            unexpected.join(", ")
        ));
    }
    for (id, decision, requirement, evidence) in REQUIRED_RULES {
        ...
        for (field, expected) in [
            ...
            expect(
                ...
                errors,
                &format!("log forwarder onboarding rule {id} {field} must match"),
            );
        }
    }
}
...
fn validate_program_text(program: &str, catalog: &Value, errors: &mut Vec<String>) {
    ...
    expect(
        exact_string_assignment(&block, "onboardingMode", "dry-run-readiness"),
        errors,
        "API must keep dry-run readiness mode",
    );
    expect(
...
fn endpoint_block(program: &str, errors: &mut Vec<String>) -> String {
    ...
    if starts.is_empty() {
        errors.push("API missing log forwarder onboarding endpoint".to_string());
        return String::new();
    }
    if starts.len() != 1 {
...
fn endpoint_payload_block(endpoint: &str, errors: &mut Vec<String>) -> String {
    ...
    if json_indexes.is_empty() {
        errors.push("API missing log forwarder onboarding JSON payload".to_string());
        return String::new();
    }
    if json_indexes.len() != 1 {
        errors
            .push("API must declare exactly one log forwarder onboarding JSON payload".to_string());
        return String::new();
    }
    ...
    let Some(object_start) = endpoint[json_index..]
        ...
        errors
            .push("API log forwarder onboarding JSON payload must be a single object".to_string());
        return String::new();
    };
...
fn validate_bound_array_immutable(
    ...
    if reassigned || mutated {
        errors.push(format!(
            "API {field} array variable {variable} must remain immutable before endpoint use"
        ));
    }
}
...
fn compact_contains_assignment(compact: &str, alias: &str) -> bool {
    compact.contains(&format!(";{alias}=new[]"))
        || compact.starts_with(&format!("{alias}=new[]"))
...
fn validate_endpoint_field_names(block: &str, errors: &mut Vec<String>) {
    for field in top_level_assignment_fields(block) {
        ...
        if prohibited_key(&field) {
            errors.push(format!(
                "API endpoint has prohibited log forwarder onboarding field {field}"
            ));
        } else {
            errors.push(format!(
                "API endpoint has unexpected log forwarder onboarding field {field}"
            ));
        }
    }
}
...
fn validate_no_unsafe_true_flags(block: &str, errors: &mut Vec<String>) {
    ...
    for field in fields {
        ...
        if assignment_has_value(&masked, &field, "true") && unsafe_true_field(&field) {
            errors.push(format!(
                "log forwarder onboarding endpoint must not enable {field}"
            ));
        }
    }
}
...
fn validate_docs_text(api_readme: &str, doc: &str, errors: &mut Vec<String>) {
    expect(
        ...
        errors,
        "API README missing log forwarder onboarding endpoint",
    );
    expect(
        doc.contains(ENDPOINT),
        errors,
        "log forwarder onboarding doc missing endpoint",
    );
    expect(
        doc.contains("No live provider calls."),
        errors,
        "log forwarder onboarding doc must prohibit provider calls",
    );
    expect(
        doc.contains("No live agent installation."),
        errors,
        "log forwarder onboarding doc must prohibit live agent install",
    );
    expect(
        doc.contains("No live configuration changes."),
        errors,
        "log forwarder onboarding doc must prohibit live config changes",
    );
    expect(
        doc.contains("No log platform mutation."),
        errors,
        "log forwarder onboarding doc must prohibit log platform mutation",
    );
    expect(
        doc.contains("log onboarding summaries only"),
        errors,
        "log forwarder onboarding doc must require safe summaries",
    );
}
