...
const CATALOG_PATH: &str = "catalog/request-execution-timeline-contract.yaml";
const API_README_PATH: &str = "api/Ryuki.Platform.Api/README.md";
const CATALOG_README_PATH: &str = "catalog/README.md";
const DOC_README_PATH: &str = "docs/workflows/README.md";
const DOC_PATH: &str = "docs/workflows/request-execution-timeline.md";
const ENDPOINT: &str = "/api/requests/execution-timeline-contract";

const REQUIRED_TIMELINE_STAGES: &[&str] = &[
    "intake",
    "validation",
    "dry-run-plan",
    "approval",
...
const REQUIRED_EVIDENCE_STATES: &[&str] = &[
    "not-required",
    "pending",
    "redacted-ready",
    "blocked",
...
const REQUIRED_TIMELINE_VIEWS: &[&str] = &[
    "request-summary",
    "timeline",
    "approval-history",
    "operation-links",
...
const SAFE_TRUE_FIELDS: &[&str] = &[
    ...
    "operationLinksReadOnly",
];
const REQUIRED_DISABLED_FIELDS: &[&str] = &[
    "liveRequestQueryAllowed",
...
const ENDPOINT_ARRAY_BINDINGS: &[(&str, &str)] = &[
    ("timelineStages", "requestExecutionTimelineStages"),
    ("timelineEventTypes", "requestExecutionTimelineEventTypes"),
    ("evidenceStates", "requestExecutionTimelineEvidenceStates"),
    ("timelineViews", "requestExecutionTimelineViews"),
    ("requiredGuards", "requestExecutionTimelineRequiredGuards"),
    ("blockedReasons", "requestExecutionTimelineBlockedReasons"),
];
...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    ...
    let context: Context = serde_json::from_str(&payload)
        .map_err(|error| format!("invalid request execution timeline context JSON: {error}"))?;
...
pub fn validate_catalog_json(input: &str) -> Result<Vec<String>, String> {
    let catalog: Value = serde_json::from_str(input)
        .map_err(|error| format!("invalid request execution timeline catalog JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_program_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProgramInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid request execution timeline program JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_docs_json(input: &str) -> Result<Vec<String>, String> {
    let payload: DocsInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid request execution timeline docs JSON: {error}"))?;
    let mut errors = Vec::new();
    validate_docs_text(
        &payload.api_readme,
...
pub fn scan_prohibited_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProhibitedInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid request execution timeline prohibited JSON: {error}"))?;
    let mut errors = Vec::new();
...
fn validate_catalog_value(catalog: &Value, errors: &mut Vec<String>) {
    let Some(map) = catalog.as_object() else {
        errors.push("request execution timeline catalog must be a mapping".to_string());
        return;
    };
    ...
    if !unexpected.is_empty() {
        errors.push(format!(
            "request execution timeline unexpected catalog keys: {}",
            unexpected.join(", ")
        ));
    }
    expect(
        ...
        errors,
        "request execution timeline version must be 1",
    );
    expect(
        string_value(catalog, "status") == Some("draft"),
        errors,
        "request execution timeline status must be draft",
    );
    expect(
        string_value(catalog, "source") == Some("static-seed"),
        errors,
        "request execution timeline source must be static-seed",
    );
    expect(
        string_value(catalog, "requestTimelineMode") == Some("static-request-execution-timeline"),
        errors,
        "request execution timeline mode must be static-request-execution-timeline",
    );
    for field in SAFE_TRUE_FIELDS {
        expect(
            bool_value(catalog, field) == Some(true),
            errors,
            format!("request execution timeline {field} must be true"),
        );
    }
    for field in REQUIRED_DISABLED_FIELDS {
        expect(
            ...
            errors,
            format!("request execution timeline {field} must be disabled"),
        );
    }
    validate_required_array(catalog, "timelineStages", REQUIRED_TIMELINE_STAGES, errors);
    validate_required_array(catalog, "timelineEventTypes", REQUIRED_EVENT_TYPES, errors);
    validate_required_array(catalog, "evidenceStates", REQUIRED_EVIDENCE_STATES, errors);
    validate_required_array(catalog, "timelineViews", REQUIRED_TIMELINE_VIEWS, errors);
...
fn validate_required_array(
    ...
    for value in values {
        if !safe_text_value(&value) && prohibited_field(&value) {
            errors.push(format!(
                "{field} contains prohibited request execution timeline value {value}"
            ));
        }
    }
}
...
fn validate_required_rules(catalog: &Value, errors: &mut Vec<String>) {
    ...
    expect(
        ...
        errors,
        format!(
            "request execution timeline missing rules: {}",
            missing.join(", ")
        ),
    );
    expect(
        ...
        errors,
        format!(
            "request execution timeline unexpected rules: {}",
            unexpected.join(", ")
        ),
    );
    expect(
        ...
        errors,
        "request execution timeline rule IDs must be unique",
    );
    expect_unique_rule_details(
        &rules,
        errors,
        "request execution timeline rule details must be unique",
    );
    for detail in REQUIRED_RULES {
        if let Some(rule) = rules.iter().find(|candidate| candidate.id == detail.id) {
            expect(
                ...
                errors,
                format!(
                    "request execution timeline rule {} decision must match",
                    detail.id
                ),
            );
            expect(
                ...
                errors,
                format!(
                    "request execution timeline rule {} requirement must match",
                    detail.id
                ),
            );
            expect(
                ...
                errors,
                format!(
                    "request execution timeline rule {} evidence must match",
                    detail.id
                ),
            );
        }
...
fn catalog_rule_hashes(catalog: &Value, errors: &mut Vec<String>) -> Vec<Rule> {
    let Some(items) = catalog.get("rules").and_then(Value::as_array) else {
        errors.push("request execution timeline rules must be an array".to_string());
        return Vec::new();
    };
    let mut rules = Vec::new();
    for item in items {
        let Some(map) = item.as_object() else {
            errors.push("request execution timeline rule entry must be a mapping".to_string());
            continue;
        };
        let unexpected: Vec<&str> = map
            .keys()
        ...
        if !unexpected.is_empty() {
            errors.push(format!(
                "request execution timeline rule {id} unexpected rule keys: {}",
                unexpected.join(", ")
            ));
        }
        if !missing.is_empty() {
            errors.push(format!(
                "request execution timeline rule {id} missing rule keys: {}",
                missing.join(", ")
            ));
        }
        rules.push(Rule {
...
fn validate_program_text(program: &str, catalog: &Value, errors: &mut Vec<String>) {
    ...
    expect(
        exact_string_assignment(
            &block,
            "requestTimelineMode",
            "static-request-execution-timeline",
        ),
        errors,
        "API must keep static-request-execution-timeline mode",
    );
    for field in SAFE_TRUE_FIELDS {
        expect(
...
fn validate_docs_text(
    ...
    expect(
        ...
        errors,
        "API README missing request execution timeline endpoint",
    );
    expect(
        catalog_readme.contains("request-execution-timeline-contract.yaml"),
        errors,
        "catalog README missing request execution timeline catalog",
    );
    expect(
        doc_readme.contains("request-execution-timeline.md"),
        errors,
        "workflow README missing request execution timeline doc",
    );
    expect(
        doc.contains(ENDPOINT),
        errors,
        "request execution timeline doc missing endpoint",
    );
    expect(
        doc.contains("No live request queries"),
        errors,
        "request execution timeline doc must prohibit live request queries",
    );
    expect(
        doc.contains("No provider calls"),
        errors,
        "request execution timeline doc must prohibit provider calls",
    );
    expect(
        doc.contains("No request, workflow, operation, provider, or notification mutation"),
        errors,
        "request execution timeline doc must prohibit mutation",
    );
    expect(
        doc.contains("raw request payloads"),
        errors,
        "request execution timeline doc must prohibit raw request payloads",
    );
    expect(
        doc.contains("raw timeline rows"),
        errors,
        "request execution timeline doc must prohibit raw timeline rows",
    );
    expect(
        doc.contains("raw approval data"),
        errors,
        "request execution timeline doc must prohibit raw approval data",
    );
    expect(
        doc.contains("raw evidence payloads"),
        errors,
        "request execution timeline doc must prohibit raw evidence payloads",
    );
    expect(
        doc.contains("raw logs"),
        errors,
        "request execution timeline doc must prohibit raw logs",
    );
    expect(
        doc.contains("raw recipient data"),
        errors,
        "request execution timeline doc must prohibit raw recipient data",
    );
    expect(
        doc.contains("static request execution timeline summaries only"),
        errors,
        "request execution timeline doc must require static summaries",
    );
}

fn endpoint_block(uncommented_program: &str, errors: &mut Vec<String>) -> String {
    let starts = endpoint_start_indexes(uncommented_program, ENDPOINT);
    let start = starts.first().copied();
    let Some(start) = start else {
        errors.push("API missing request execution timeline endpoint".to_string());
        return String::new();
    };
    expect(
        starts.len() == 1,
        errors,
        "API must expose exactly one request execution timeline endpoint",
    );
    let mut next = uncommented_program.len();
    let mut scan_offset = start + 1;
    for line in uncommented_program[start + 1..].split_inclusive('\n') {
        if endpoint_registration_line(line, None) {
            next = scan_offset;
            break;
        }
...
fn validate_endpoint_field_names(block: &str, errors: &mut Vec<String>) {
    for field in assignment_fields(&strip_csharp_string_literals(block)) {
        if !endpoint_field_allowed(&field) {
            errors.push(format!(
                "API endpoint has unexpected request execution timeline field {field}"
            ));
            continue;
        }
        if prohibited_field(&field) {
            errors.push(format!(
                "API endpoint has prohibited request execution timeline field {field}"
            ));
        }
    }
}
        ...
        Value::Object(map) => {
            for (key, child) in map {
                if prohibited_field(key) {
                    errors.push(format!(
                        "{path}.{key} contains prohibited request execution timeline field"
                    ));
                }
        ...
        Value::String(text) => {
            if whole_file_text(path, text) {
                ...
                if prohibited_field(text) {
                    errors.push(format!(
                        "{path} contains prohibited request execution timeline value {text}"
                    ));
                }
            }
        }
...
mod tests {
    ...
    fn prohibited_literal_scan_rejects_synthetic_values() {
        ...
        assert!(prohibited_value("https://request-timeline.invalid/run"));
    ...
    fn csharp_array_lookup_uses_exact_identifier() {
        let program = r#"
var requestExecutionTimelineEventTypesShadow = new[] { "request-state" };
var requestExecutionTimelineEventTypes = new[] { "unsafe-event" };
"#;
        assert_eq!(
            csharp_array_values(program, "requestExecutionTimelineEventTypes"),
            Some(vec!["unsafe-event".to_string()])
        );
    }

    #[test]
    fn spaced_duplicate_endpoint_registration_is_counted() {
        let program = format!(
            "app . MapGet(\"{ENDPOINT}\", () => Results.Json(new {{ }}));\napp.MapGet(\"{ENDPOINT}\", () => Results.Json(new {{ }}));"
        );
        let uncommented = csharp_without_comments(&program);

        assert_eq!(endpoint_start_indexes(&uncommented, ENDPOINT).len(), 2);
    }
}
