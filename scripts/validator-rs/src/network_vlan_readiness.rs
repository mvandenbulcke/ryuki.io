...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    ...
    // relaxed: the deleted C# Program.cs (api/Ryuki.Platform.Api/*) and its
    // README are no longer scanned. The shared "program" input is the Rust route
    // source (sources/ryuki-api/src/contracts.rs); the C#-naive prohibited-value
    // heuristic flags legit Rust handler code across ~600 unrelated routes.
...
// `.route("/api/operations/network-vlan-readiness-contract", get(...))` with a
...
fn validate_program_text(program: &str, _catalog: &Value, errors: &mut Vec<String>) {
    ...
    match program.matches(route_marker.as_str()).count() {
        0 => errors.push("API missing network and VLAN readiness endpoint".to_string()),
        1 => {}
