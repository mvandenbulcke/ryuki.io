...
const CATALOG_PATH: &str = "catalog/synthetic-health-check-contract.yaml";
const PROGRAM_PATH: &str = "api/Ryuki.Platform.Api/Program.cs";
const API_README_PATH: &str = "api/Ryuki.Platform.Api/README.md";
const DOC_PATH: &str = "docs/workflows/synthetic-health-checks.md";
...
const REQUIRED_RULES: &[(&str, &str, &str, &str)] = &[
    (
        "no-live-synthetic-probes",
        "block",
        "Synthetic health checks produce definition drafts only, never running live probes or calling external endpoints.",
        "Synthetic definition draft",
    ),
    (
        "no-zabbix-mutation",
        "block",
        "Synthetic definitions do not create, update, or delete Zabbix checks until live integration gates exist.",
        "Synthetic definition draft",
    ),
    (
        "target-scope-required",
        "block",
        "Target scope must be reviewed and aggregate-safe before a synthetic check can be drafted.",
        "Target scope summary",
    ),
    (
        "maintenance-impact-required",
        "block",
        "Maintenance-window and alert-impact behavior must be reviewed before check definitions are approved.",
        "Maintenance impact",
    ),
    (
        "raw-probe-output-not-exposed",
        "block",
        "Operators receive synthetic check summaries only, not raw probe output, alert payloads, certificate serials, or provider output.",
        "Synthetic check summary",
    ),
];
const ENDPOINT_ARRAY_BINDINGS: &[(&str, &str, &[&str])] = &[
    (
        "supportedWorkflows",
        "syntheticHealthWorkflows",
        REQUIRED_WORKFLOWS,
    ),
    ("checkSignals", "syntheticHealthSignals", REQUIRED_SIGNALS),
    (
        "requiredGuards",
        "syntheticHealthRequiredGuards",
        REQUIRED_GUARDS,
    ),
    (
        "planSections",
        "syntheticHealthPlanSections",
        REQUIRED_PLAN_SECTIONS,
    ),
    (
        "blockedReasons",
        "syntheticHealthBlockedReasons",
...
const ALLOWED_ENDPOINT_FIELDS: &[&str] = &[
    ...
    "liveChecksAllowed",
    "externalProbesAllowed",
    "zabbixMutationAllowed",
    "rawProbeOutputAllowed",
    "supportedWorkflows",
    "checkSignals",
    "requiredInputs",
    "requiredGuards",
...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    let input = fs::read_to_string(path)
        .map_err(|error| format!("failed to read synthetic health check context: {error}"))?;
...
pub fn validate_program_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProgramInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid synthetic health check program JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_docs_json(input: &str) -> Result<Vec<String>, String> {
    let payload: DocsInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid synthetic health check docs JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn scan_prohibited_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ScanInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid synthetic health check scan JSON: {error}"))?;
    let mut errors = Vec::new();
...
fn validate_catalog_value(catalog: &Value, errors: &mut Vec<String>) {
    ...
    expect(
        string_value(catalog, "checkMode") == Some("dry-run-definition"),
        errors,
        "synthetic health check mode must be dry-run-definition",
    );
    expect(
...
fn validate_required_rules(catalog: &Value, errors: &mut Vec<String>) {
    let rules = object_array(catalog.get("rules"), "synthetic health check rule", errors);
    let rule_ids = rules
        .iter()
    ...
    expect(
        unique(&rule_ids),
        errors,
        "synthetic health check rule IDs must be unique",
    );
    ...
    validate_rule_detail_uniqueness_value(&rules, "synthetic health check catalog", errors);
    for (id, decision, requirement, evidence) in REQUIRED_RULES {
        ...
        for (field, expected) in [
            ...
            expect(
                ...
                errors,
                &format!("synthetic health check rule {id} {field} must match"),
            );
        }
    }
}
...
fn validate_program_text(program: &str, catalog: &Value, errors: &mut Vec<String>) {
    ...
    expect(
        exact_string_assignment(&block, "checkMode", "dry-run-definition"),
        errors,
        "API must keep dry-run-definition mode",
    );
    for (field, value, message) in [
        ...
        (
            "liveChecksAllowed",
            "false",
            "API must keep liveChecksAllowed disabled",
        ),
        (
            "externalProbesAllowed",
            "false",
            "API must keep externalProbesAllowed disabled",
        ),
        (
            "zabbixMutationAllowed",
            "false",
        ...
        (
            "rawProbeOutputAllowed",
            "false",
            "API must keep rawProbeOutputAllowed disabled",
        ),
    ] {
...
fn endpoint_block(program: &str, errors: &mut Vec<String>) -> String {
    ...
    if starts.is_empty() {
        errors.push("API missing synthetic health check endpoint".to_string());
        return String::new();
    }
    if starts.len() != 1 {
...
fn endpoint_payload_block(endpoint: &str, errors: &mut Vec<String>) -> String {
    ...
    if all_json.len() > 1 {
        errors.push("API must declare exactly one synthetic health check JSON payload".to_string());
        return String::new();
    }
    let json_indexes = results_json_indexes(endpoint, true);
    if json_indexes.is_empty() {
        if all_json.is_empty() {
            errors.push("API missing synthetic health check JSON payload".to_string());
        } else {
            errors.push(
                "API synthetic health check JSON payload must use anonymous Results.Json(new { ... })"
                    .to_string(),
            );
        }
        return String::new();
    ...
    let Some(object_start) = endpoint[json_index..]
        ...
        errors.push("API synthetic health check JSON payload must be a single object".to_string());
        return String::new();
    };
    ...
    if endpoint[object_end + 1..].trim() != "));" {
        errors.push(
            "API synthetic health check JSON payload must be static anonymous object with no extra JSON arguments"
                .to_string(),
        );
        return String::new();
    }
...
fn validate_api_rules(block: &str, catalog: &Value, errors: &mut Vec<String>) {
    ...
    let catalog_rules = object_array(catalog.get("rules"), "synthetic health check rule", errors);
    ...
    validate_rule_detail_uniqueness_map(&api_rules, "synthetic health check API", errors);
...
fn validate_endpoint_field_names(block: &str, errors: &mut Vec<String>) {
    for field in top_level_assignment_fields(block) {
        ...
        if prohibited_provider_key(&field, false) {
            errors.push(format!(
                "API endpoint has prohibited synthetic health check field {field}"
            ));
        } else {
            errors.push(format!(
                "API endpoint has unexpected synthetic health check field {field}"
            ));
        }
    }
...
fn validate_no_unsafe_true_flags(block: &str, errors: &mut Vec<String>) {
    ...
    for field in assignment_fields(block) {
        let any_true = top_level_assignment_texts(&masked, &field)
            .iter()
            .chain(assignment_texts_any_depth(block, &field).iter())
...
fn validate_docs_text(readme: &str, doc: &str, errors: &mut Vec<String>) {
    expect(
        ...
        errors,
        "API README missing synthetic health check endpoint",
    );
    expect(
        doc.contains(ENDPOINT),
        errors,
        "synthetic health check doc missing endpoint",
    );
    expect(
        doc.contains("No live provider calls."),
        errors,
        "synthetic health check doc must prohibit provider calls",
    );
    expect(
        doc.contains("No live health checks or external probes."),
        errors,
        "synthetic health check doc must prohibit live probes",
    );
    expect(
        doc.contains("No Zabbix mutation."),
        errors,
        "synthetic health check doc must prohibit Zabbix mutation",
    );
    expect(
        doc.contains("synthetic check summaries only"),
        errors,
        "synthetic health check doc must require safe summaries",
    );
}
