...
const CATALOG_PATH: &str = "catalog/standard-task-contract.yaml";
const PROGRAM_PATH: &str = "api/Ryuki.Platform.Api/Program.cs";
const API_README_PATH: &str = "api/Ryuki.Platform.Api/README.md";
const CATALOG_README_PATH: &str = "catalog/README.md";
const DOC_README_PATH: &str = "docs/workflows/README.md";
const DOC_PATH: &str = "docs/workflows/standard-tasks.md";
const ENDPOINT: &str = "/api/operations/standard-task-contract";

const REQUIRED_TASK_TYPES: &[&str] = &[
    "health-check-plan",
    "collect-logs-plan",
...
const REQUIRED_DISABLED_FIELDS: &[&str] = &[
    ...
    "liveServiceChangesAllowed",
    "liveDiskChangesAllowed",
    "liveBackupActionsAllowed",
    "liveAlertSuppressionAllowed",
    "rawTargetDataAllowed",
    "rawLogContentAllowed",
    "rawRecipientDataAllowed",
    "rawWorkerPayloadsAllowed",
...
const ENDPOINT_ARRAY_BINDINGS: &[(&str, &str)] = &[
    ...
    ("taskScopes", "standardTaskScopes"),
    ("requiredGuards", "standardTaskRequiredGuards"),
    ("planSections", "standardTaskPlanSections"),
    ("blockedReasons", "standardTaskBlockedReasons"),
];
...
const SAFE_TEXT_PROHIBITION_LINES: &[&str] = &[
    ...
    "- No hostnames, usernames, raw recipient data, target identifiers, service names, file paths, credential values, secret values, access tokens, tenant identifiers, object identifiers, endpoint names, private network details, raw log content, raw target data, worker payloads, or provider payloads in committed files.",
    "| `/api/operations/standard-task-contract` | Static standard L1/L2 task contract; worker execution, live provider actions, and raw task data disabled. |",
...
const STATIC_SAFE_VALUES: &[&str] = &["draft", "static-seed", "dry-run-plan-only", "block"];
...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    ...
    let context: StandardTaskContext = serde_json::from_str(&payload)
        .map_err(|error| format!("invalid standard task context JSON: {error}"))?;
...
pub fn validate_catalog_json(input: &str) -> Result<Vec<String>, String> {
    let catalog: Value = serde_json::from_str(input)
        .map_err(|error| format!("invalid standard task catalog JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_program_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProgramInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid standard task program JSON: {error}"))?;
    let mut errors = Vec::new();
...
pub fn validate_docs_json(input: &str) -> Result<Vec<String>, String> {
    let payload: DocsInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid standard task docs JSON: {error}"))?;
    let mut errors = Vec::new();
    validate_docs_text(
        &payload.api_readme,
...
pub fn scan_prohibited_json(input: &str) -> Result<Vec<String>, String> {
    let payload: ProhibitedInput = serde_json::from_str(input)
        .map_err(|error| format!("invalid standard task prohibited JSON: {error}"))?;
    let mut errors = Vec::new();
...
fn validate_catalog_value(catalog: &Value, errors: &mut Vec<String>) {
    let Some(map) = catalog.as_object() else {
        errors.push("standard task catalog must be a mapping".to_string());
        return;
    };
    ...
    if !unexpected.is_empty() {
        errors.push(format!(
            "standard task unexpected catalog keys: {}",
            unexpected.join(", ")
        ));
    }
    expect(
        ...
        errors,
        "standard task version must be 1",
    );
    expect(
        string_value(catalog, "status") == Some("draft"),
        errors,
        "standard task status must be draft",
    );
    expect(
        string_value(catalog, "source") == Some("static-seed"),
        errors,
        "standard task source must be static-seed",
    );
    expect(
        string_value(catalog, "taskMode") == Some("dry-run-plan-only"),
        errors,
        "standard task mode must be dry-run-plan-only",
    );
    expect(
        bool_value(catalog, "dryRunRequired") == Some(true),
        errors,
        "standard task must require dry-run",
    );
    for field in REQUIRED_DISABLED_FIELDS {
        expect(
            bool_value(catalog, field) == Some(false),
            errors,
            format!("standard task {field} must be disabled"),
        );
    }
    validate_required_array(catalog, "taskTypes", REQUIRED_TASK_TYPES, errors);
    validate_required_array(catalog, "taskScopes", REQUIRED_SCOPES, errors);
...
fn validate_required_rules(catalog: &Value, errors: &mut Vec<String>) {
    ...
    if !missing.is_empty() {
        errors.push(format!(
            "standard task missing rules: {}",
            missing.join(", ")
        ));
    }
    if !unexpected.is_empty() {
        errors.push(format!(
            "standard task unexpected rules: {}",
            unexpected.join(", ")
        ));
    }
    expect(
        ...
        errors,
        "standard task rule IDs must be unique",
    );
    expect(
        unique_len(
            &rules
        ...
        errors,
        "standard task rule requirements must be unique",
    );
    expect(
        unique_len(
            &rules
        ...
        errors,
        "standard task rule evidence must be unique",
    );
    let details = rules
        .iter()
    ...
    expect(
        ...
        errors,
        "standard task rule details must be unique",
    );
    for expected in REQUIRED_RULES {
        ...
        expect(
            ...
            errors,
            format!("standard task rule {} decision must match", expected.id),
        );
        expect(
            rule.requirement == expected.requirement,
            errors,
            format!("standard task rule {} requirement must match", expected.id),
        );
        expect(
            rule.evidence == expected.evidence,
            errors,
            format!("standard task rule {} evidence must match", expected.id),
        );
    }
}
...
fn validate_program_text(program: &str, catalog: &Value, errors: &mut Vec<String>) {
    ...
    expect(
        exact_string_assignment(&block, "taskMode", "dry-run-plan-only")
            && endpoint_assignment_count(&block, "taskMode") == 1,
        errors,
        "API must keep exactly one dry-run-plan-only mode",
    );
    expect(
        exact_endpoint_assignment(&block, "dryRunRequired", "true")
            && endpoint_assignment_count(&block, "dryRunRequired") == 1,
        errors,
        "API must require exactly one dry-run",
    );
    for field in REQUIRED_DISABLED_FIELDS {
        expect(
    ...
    for (field, variable) in ENDPOINT_ARRAY_BINDINGS {
        ...
        let values = csharp_array_values(program, variable, &format!("API {variable}"), errors);
...
fn validate_api_array(
    ...
    for value in values {
        ...
        if prohibited_field(&value) {
            errors.push(format!(
                "API {field} contains prohibited standard task value {value}"
            ));
        }
        if let Some(phrase) = prohibited_phrase(&value) {
            errors.push(format!(
                "API {field} contains prohibited standard task phrase {phrase}"
            ));
        }
    }
}
...
fn validate_endpoint_field_names(block: &str, errors: &mut Vec<String>) {
    ...
    for field in assignment_fields(&surface) {
        if !ALLOWED_ENDPOINT_FIELDS.contains(&field.as_str()) {
            errors.push(format!(
                "API endpoint has unexpected standard task field {field}"
            ));
            continue;
        }
        if prohibited_field(&field) {
            errors.push(format!(
                "API endpoint has prohibited standard task field {field}"
            ));
        }
    }
}
...
fn validate_endpoint_no_prohibited_literals(block: &str, errors: &mut Vec<String>) {
    for value in csharp_string_literals(block) {
        ...
        if let Some(phrase) = prohibited_phrase(&value) {
            errors.push(format!(
                "API endpoint contains prohibited standard task phrase {phrase}"
            ));
        }
        if prohibited_field(&value) {
            errors.push(format!(
                "API endpoint contains prohibited standard task value {value}"
            ));
        }
    }
}
...
fn validate_docs_text(
    ...
    expect(
        ...
        errors,
        "API README missing standard task endpoint",
    );
    expect(
        catalog_readme.contains("standard-task-contract.yaml"),
        errors,
        "catalog README missing standard task catalog",
    );
    expect(
        doc_readme.contains("standard-tasks.md"),
        errors,
        "workflow README missing standard task doc",
    );
    expect(
        doc.contains(ENDPOINT),
        errors,
        "standard task doc missing endpoint",
    );
    expect(
        doc.contains("No live provider calls."),
        errors,
        "standard task doc must prohibit provider calls",
    );
    expect(
        doc.contains("No worker execution."),
        errors,
        "standard task doc must prohibit worker execution",
    );
    expect(
        doc.contains("No live service changes."),
        errors,
        "standard task doc must prohibit service changes",
    );
    expect(
        doc.contains("No live disk changes."),
        errors,
        "standard task doc must prohibit disk changes",
    );
    expect(
        doc.contains("No live backup actions."),
        errors,
        "standard task doc must prohibit backup actions",
    );
    expect(
        doc.contains("No live alert suppression."),
        errors,
        "standard task doc must prohibit alert suppression",
    );
    expect(
        doc.contains("static standard task summaries only"),
        errors,
        "standard task doc must require static summaries",
    );
}
...
fn endpoint_block(program: &str, errors: &mut Vec<String>) -> String {
    ...
    let matches = routes
        ...
        .collect::<Vec<_>>();
    if matches.is_empty() {
        errors.push("API missing standard task endpoint".to_string());
        return String::new();
    }
    if matches.len() != 1 {
        errors.push(format!(
...
fn csharp_array_values(
    ...
    let scan_program = mask_csharp_string_literals(program);
    let open = scan_program
        .get(start..)?
        .find('{')
        .map(|offset| start + offset)?;
    let close = matching_delimiter_index(&scan_program, open, b'{', b'}')?;
    program
        .get((open + 1)..close)
    ...
    match value {
        Value::Object(map) => {
            for (key, child) in map {
                if prohibited_field(key) {
                    errors.push(format!(
                        "{path}.{key} contains prohibited standard task field"
                    ));
                }
        ...
        Value::String(text) => {
            ...
            if let Some(phrase) = prohibited_phrase(text) {
                errors.push(format!(
                    "{path} contains prohibited standard task phrase {phrase}"
                ));
            }
            if prohibited_field(text) {
                errors.push(format!(
                    "{path} contains prohibited standard task value {text}"
                ));
            }
        }
        _ => {}
...
fn validate_text_terms(text: &str, path: &str, errors: &mut Vec<String>) {
    if !standard_task_text_path(path) {
        return;
    }
    for (index, line) in text.lines().enumerate() {
        if !standard_task_text_line(path, line) || safe_text_line(line) {
            continue;
        }
        ...
        for term in words(line) {
            if prohibited_field(&term) {
                errors.push(format!(
                    "{path}:{} contains prohibited standard task field {term}",
                    index + 1
                ));
            }
        }
...
fn standard_task_text_path(path: &str) -> bool {
    [
        CATALOG_PATH,
        DOC_PATH,
        API_README_PATH,
...
fn standard_task_text_line(path: &str, line: &str) -> bool {
    path.ends_with(CATALOG_PATH)
        || path.ends_with(DOC_PATH)
        || line.to_ascii_lowercase().contains("standard-task")
        || line.to_ascii_lowercase().contains("standard task")
        || line.contains("L1/L2")
...
fn safe_text_value(value: &str) -> bool {
    STATIC_SAFE_VALUES.contains(&value)
        || REQUIRED_TASK_TYPES.contains(&value)
...
mod tests {
    ...
    #[test]
    fn scan_prohibited_json_rejects_identifier_keys() {
        let input = serde_json::json!({
            "value": {
                "customerId": "safe-summary"
            },
            "path": "synthetic"
        })
        .to_string();

        let errors = scan_prohibited_json(&input).expect("valid scan payload");

        assert!(errors.iter().any(|error| error.contains("customerId")));
    }

    #[test]
    fn rule_literal_assignments_are_parsed() {
        let body = r#" id = "no-live-standard-task-execution", decision = "block", requirement = "One, two, three.", evidence = "Task plan summary" "#;

        assert_eq!(
            assignment_value(body, "id"),
            Some(r#""no-live-standard-task-execution""#)
        );
        assert_eq!(
            literal_assignment(body, "id").as_deref(),
            Some("no-live-standard-task-execution")
        );
        assert_eq!(
