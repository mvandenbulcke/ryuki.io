...
pub fn validate_context_file(path: &Path) -> Result<Vec<String>, String> {
    ...
    // and `{id}` path params belonging to unrelated endpoints. The decommission
    // quarantine handler payload is scanned for live safety flags in
...
// decommission quarantine contract is mounted as `.route(ENDPOINT,
...
fn validate_program_text(program: &str, _catalog: &Value, errors: &mut Vec<String>) {
    let Some(payload) = crate::rust_contract::endpoint_payload(
        program,
        ENDPOINT,
        "API missing VM decommission quarantine endpoint",
        "API missing VM decommission quarantine JSON payload",
        errors,
    ) else {
        return;
    };
